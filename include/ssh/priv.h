 ...
 * without notice; ssh-api.h file will keep backward compatibility
 * on binary & source
 */

#ifndef SSH_LIBSSH_PRIV_H
#define SSH_LIBSSH_PRIV_H
...
/* LOGGING */
SSH_API void ssh_log_function(int verbosity,
                      const char *function,
                      const char *buffer);
#define SSH_INFO(priority, ...) \
    _ssh_log(priority, __FUNCTION__, __VA_ARGS__)

/* LEGACY */
SSH_API void ssh_log_common(struct ssh_common_struct *common,
                    int verbosity,
                    const char *function,
...
SSH_API void _ssh_set_error(void *error,
                    int code,
                    const char *function,
...
SSH_API void _ssh_set_error_oom(void *error, const char *function);

#define ssh_set_error_invalid(error) \
    _ssh_set_error_invalid(error, __FUNCTION__)
SSH_API void _ssh_set_error_invalid(void *error, const char *function);

SSH_API int ssh_get_key_params(ssh_session_t * session, ssh_key_t * *privkey);

/* server.c */
#ifdef WITH_SERVER
SSH_API int ssh_auth_reply_default(ssh_session_t * session,int partial);
SSH_API int ssh_auth_reply_success(ssh_session_t * session, int partial);
#endif
/* client.c */

SSH_API int ssh_send_banner(ssh_session_t * session, int is_server);

/* connect.c */
SSH_API socket_t ssh_connect_host(ssh_session_t * session, const char *host,const char
        *bind_addr, int port, long timeout, long usec);
SSH_API socket_t ssh_connect_host_nonblocking(ssh_session_t * session, const char *host,
		const char *bind_addr, int port);

/* in base64.c */
SSH_API ssh_buffer_t * base64_to_bin(const char *source);
SSH_API unsigned char *bin_to_base64(const unsigned char *source, int len);

/* gzip.c */
SSH_API int compress_buffer(ssh_session_t * session,ssh_buffer_t * buf);
SSH_API int decompress_buffer(ssh_session_t * session,ssh_buffer_t * buf, size_t maxlen);

/* match.c */
SSH_API int match_hostname(const char *host, const char *pattern, unsigned int len);
SSH_API int match_pattern_list(const char *string, const char *pattern,
