...
static void create_files(ssh_session_t *session){
...
static int fetch_files(ssh_session_t *session){
  int size;
  char buffer[16384];
  int mode;
  char *filename;
  int r;
  ssh_scp_t *scp=ssh_scp_new(session, SSH_SCP_READ | SSH_SCP_RECURSIVE, "/tmp/libssh_tests/*");
