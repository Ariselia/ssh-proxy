...
SSH_PACKET_CALLBACK(ssh_packet_kexinit){
  ...
  if (session->session_state == SSH_SESSION_STATE_AUTHENTICATED){
      SSH_INFO(SSH_LOG_WARNING, "Other side initiating key re-exchange");
...
void ssh_list_kex(ssh_kex_t *kex) {
  int i = 0;
  ...
  for(i = 0; i < SSH_KEX_METHODS; i++) {
    ...
    SSH_INFO(SSH_LOG_FUNCTIONS, "%s: %s",
 ...
 */
static char *ssh_client_select_hostkeys(ssh_session_t * session){
	...
	if(strlen(methods_buffer) > 0){
		SSH_INFO(SSH_LOG_DEBUG, "Changing host key method to \"%s\"", methods_buffer);
		return strdup(methods_buffer);
	} else {
		SSH_INFO(SSH_LOG_DEBUG, "No supported kex method for existing key in known_hosts file");
		return NULL;
	}
 ...
 */
int set_client_kex(ssh_session_t * session){
    ssh_kex_t *client= &session->next_crypto->client_kex;
    const char *wanted;
    int i;
 ...
 */
int ssh_kex_select_methods (ssh_session_t * session){
...
int ssh_send_kex(ssh_session_t * session, int server_kex) {
  ssh_kex_t *kex = (server_kex ? &session->next_crypto->server_kex :
