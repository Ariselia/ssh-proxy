 ...
 */
static int ssh_gssapi_init(ssh_session_t * session){
 ...
 */
static void ssh_gssapi_free(ssh_session_t * session){
    OM_uint32 min;
    if (session->gssapi == NULL)
        return;
 ...
 */
static int ssh_gssapi_send_response(ssh_session_t * session, ssh_string_t * oid){
...
static void ssh_gssapi_log_error(int verb, const char *msg, int maj_stat){
    ...
    SSH_INFO(verb, "GSSAPI(%s): %s", msg, (const char *)buffer.value);
}
...
int ssh_gssapi_handle_userauth(ssh_session_t * session, const char *user, uint32_t n_oid, ssh_string_t * *oids){
    ...
    if (ssh_callbacks_exists(session->server_callbacks, gssapi_select_oid_function)){
        ssh_string_t * oid_s = session->server_callbacks->gssapi_select_oid_function(session,
                user, n_oid, oids,
    ...
    for (i=0; i < supported->count; ++i){
        ...
        SSH_INFO(SSH_LOG_DEBUG, "Supported mech %d: %s\n", i, ptr);
        free(ptr);
    }
    ...
    for (i=0 ; i< n_oid ; ++i){
        ...
        if(len < 2 || oid_s[0] != SSH_OID_TAG || ((size_t)oid_s[1]) != len - 2){
            SSH_INFO(SSH_LOG_WARNING,"GSSAPI: received invalid OID");
            continue;
        }
    ...
    if (oid_count == 0){
        SSH_INFO(SSH_LOG_PROTOCOL,"GSSAPI: no OID match");
    ...
    if (maj_stat != GSS_S_COMPLETE) {
        SSH_INFO(SSH_LOG_WARNING, "importing name %d, %d", maj_stat, min_stat);
    ...
    if (maj_stat != GSS_S_COMPLETE) {
        SSH_INFO(SSH_LOG_WARNING, "error acquiring credentials %d, %d", maj_stat, min_stat);
        ssh_gssapi_log_error(SSH_LOG_WARNING, "acquiring creds", maj_stat);
        ssh_auth_reply_default(session,0);
        return SSH_ERROR;
    }

    SSH_INFO(SSH_LOG_PROTOCOL, "acquiring credentials %d, %d", maj_stat, min_stat);
    ...
    for (i=0 ; i< n_oid ; ++i){
        ...
        if(present){
            SSH_INFO(SSH_LOG_PACKET, "Selected oid %d", i);
            break;
        }
    }
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_gssapi_token_server){
    ...
    SSH_INFO(SSH_LOG_PACKET,"Received SSH_MSG_USERAUTH_GSSAPI_TOKEN");
    ...
    SSH_INFO(SSH_LOG_PACKET, "GSSAPI Token : %s",hexa);
    SAFE_FREE(hexa);
    ...
    if (output_token.length != 0){
        ...
        SSH_INFO(SSH_LOG_PACKET, "GSSAPI: sending token %s",hexa);
        SAFE_FREE(hexa);
...
static ssh_buffer_t * ssh_gssapi_build_mic(ssh_session_t * session){
    ssh_buffer_t * mic_buffer;
    ssh_string_t * str;
    int rc;
...
{
    ssh_string_t * mic_token;
    OM_uint32 maj_stat, min_stat;
    gss_buffer_desc mic_buf = GSS_C_EMPTY_BUFFER;
    gss_buffer_desc mic_token_buf = GSS_C_EMPTY_BUFFER;
    ssh_buffer_t * mic_buffer = NULL;

    (void)user;
    (void)type;

    SSH_INFO(SSH_LOG_PACKET,"Received SSH_MSG_USERAUTH_GSSAPI_MIC");
 ...
 */
ssh_gssapi_creds ssh_gssapi_get_creds(ssh_session_t * session){
 ...
 */
void ssh_gssapi_set_creds(ssh_session_t * session, const ssh_gssapi_creds creds)
{
    if (session == NULL) {
        return;
    }
...
static int ssh_gssapi_send_auth_mic(ssh_session_t * session, ssh_string_t * *oid_set, int n_oid){
    ssh_string_t * str;
    int rc;
    int i;
 ...
 */
static int ssh_gssapi_match(ssh_session_t * session, gss_OID_set *valid_oids)
{
    ...
    for (i = 0; i < actual_mechs->count; i++) {
        ...
        if (maj_stat == GSS_S_COMPLETE && lifetime > 0) {
            ...
            SSH_INFO(SSH_LOG_DEBUG, "GSSAPI valid oid %d : %s\n", i, ptr);
            SAFE_FREE(ptr);
        }
    }
...
int ssh_gssapi_auth_mic(ssh_session_t * session){
    ...
    SSH_INFO(SSH_LOG_PROTOCOL, "Authenticating with gssapi to host %s with user %s",
    ...
    SSH_INFO(SSH_LOG_PROTOCOL, "Sending %d oids", n_oids);

    oids = calloc(n_oids, sizeof(ssh_string_t *));
    if (oids == NULL) {
        ssh_set_error_oom(session);
        return SSH_AUTH_ERROR;
    }
...
static gss_OID ssh_gssapi_oid_from_string(ssh_string_t * oid_s){
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_gssapi_response){
    ...
    SSH_INFO(SSH_LOG_PACKET, "Received SSH_USERAUTH_GSSAPI_RESPONSE");
...
static int ssh_gssapi_send_mic(ssh_session_t * session){
    OM_uint32 maj_stat, min_stat;
    gss_buffer_desc mic_buf = GSS_C_EMPTY_BUFFER;
    gss_buffer_desc mic_token_buf = GSS_C_EMPTY_BUFFER;
    ssh_buffer_t * mic_buffer;
    ...
    SSH_INFO(SSH_LOG_PACKET,"Sending SSH_MSG_USERAUTH_GSSAPI_MIC");
