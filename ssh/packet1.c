...
int ssh_packet_socket_callback1(const void *data, size_t receivedlen, void *user) {
  ...
  switch (session->packet_state){
    case PACKET_STATE_INIT:
      ...
      SSH_INFO(SSH_LOG_PACKET, "Reading a %d bytes packet", len);
    ...
    case PACKET_STATE_SIZEREAD:
      ...
      ssh_print_hexa("read packet decrypted:", ssh_buffer_get_begin(session->in_buffer),
          ssh_buffer_get_len(session->in_buffer));
#endif
      SSH_INFO(SSH_LOG_PACKET, "%d bytes padding", padding);
      if(((len + padding) != buffer_get_rest_len(session->in_buffer)) ||
          ((len + padding) < sizeof(uint32_t))) {
        SSH_INFO(SSH_LOG_RARE, "no crc32 in packet");
      ...
      if (ssh_crc32(buffer_get_rest(session->in_buffer),
        ...
        ssh_print_hexa("crc32 on",buffer_get_rest(session->in_buffer),
            len + padding - sizeof(uint32_t));
#endif
        SSH_INFO(SSH_LOG_RARE, "Invalid crc32");
        ssh_set_error(session, SSH_FATAL,
      ...
      SSH_INFO(SSH_LOG_PACKET, "The packet is valid");
...
*/
      ...
      if(processed < receivedlen){
        ...
        SSH_INFO(SSH_LOG_PACKET,"Processing %" PRIdS " bytes left in socket buffer",
...
int packet_send1(ssh_session_t * session) {
  ...
  SSH_INFO(SSH_LOG_PACKET,"Sending a %d bytes long packet",currentlen);
	// WANGFENG: proxy
...
*/
  ...
  if(session->data_send) {
  	rc = session->data_send(session, command, ssh_buffer_get_begin(session->out_buffer),
      ssh_buffer_get_len(session->out_buffer));
  } else {
...
SSH_PACKET_CALLBACK(ssh_packet_disconnect1){
  ...
  (void)user;
  (void)type;
  SSH_INFO(SSH_LOG_PACKET, "Received SSH_MSG_DISCONNECT");
