 ...
 * @returns an empty ssh_key_t * handle, or NULL on error.
 */
ssh_key_t * ssh_key_new (void) {
  ssh_key_t * ptr = malloc (sizeof (struct ssh_key_struct));
  if (ptr == NULL) {
      return NULL;
  }
  ZERO_STRUCTP(ptr);
...
ssh_key_t * ssh_key_dup(const ssh_key_t * key)
{
    if (key == NULL) {
        return NULL;
    }
 ...
 * @param[in] key ssh_key_t * to clean
 */
void ssh_key_clean (ssh_key_t * key){
    if(key == NULL)
        return;
 ...
 * @param[in] key ssh_key_t * handle to free
 */
 ...
 * @param[in] key the ssh_key_t * handle
 * @returns one of SSH_KEYTYPE_RSA,SSH_KEYTYPE_DSS,SSH_KEYTYPE_RSA1
 * @returns SSH_KEYTYPE_UNKNOWN if the type is unknown
 */
enum ssh_keytypes_e ssh_key_type(const ssh_key_t * key){
    if (key == NULL) {
        return SSH_KEYTYPE_UNKNOWN;
    }
    return key->type;
 ...
 */
int ssh_key_is_public(const ssh_key_t * k) {
    if (k == NULL) {
        return 0;
    }
 ...
 */
int ssh_key_is_private(const ssh_key_t * k) {
    if (k == NULL) {
        return 0;
    }
 ...
 */
int ssh_key_cmp(const ssh_key_t * k1,
                const ssh_key_t * k2,
 ...
 * @param[in]  auth_data Private data passed to the auth function.
 *
 * @param[out] pkey     A pointer to store the allocated ssh_key_t *. You need to
 *                      free the key.
 *
 ...
 */
int ssh_pki_export_privkey_file(const ssh_key_t * privkey,
                                const char *passphrase,
                                ssh_auth_callback auth_fn,
                                void *auth_data,
                                const char *filename)
{
    ...
    if (fp == NULL) {
        SSH_INFO(SSH_LOG_FUNCTIONS, "Error opening %s: %s",
                filename, strerror(errno));
        return SSH_EOF;
    }
...
/* temporary function to migrate seemlessly to ssh_key_t * */
ssh_public_key ssh_pki_convert_key_to_publickey(const ssh_key_t * key) {
    ssh_public_key pub;
    ssh_key_t * tmp;
...
ssh_private_key ssh_pki_convert_key_to_privatekey(const ssh_key_t * key) {
...
static int pki_import_pubkey_buffer(ssh_buffer_t * buffer,
 ...
 * @see ssh_key_free()
 */
int ssh_pki_import_pubkey_blob(const ssh_string_t * key_blob,
                               ssh_key_t * *pkey) {
    ssh_buffer_t * buffer;
    ssh_string_t * type_s = NULL;
    enum ssh_keytypes_e type;
    int rc;
 ...
 * @see ssh_key_free()
 */
int ssh_pki_import_pubkey_file(const char *filename, ssh_key_t * *pkey)
{
    enum ssh_keytypes_e type;
    struct stat sb;
    char *key_buf, *p;
 ...
 * @see ssh_key_free()
 */
int ssh_pki_export_privkey_to_pubkey(const ssh_key_t * privkey,
                                     ssh_key_t * *pkey)
{
    ssh_key_t * pubkey;
 ...
 *
 * @param[in]  key      A public or private key to create the public ssh_string_t *
 *                      from.
 *
 ...
 * @see ssh_string_free()
 */
int ssh_pki_export_pubkey_blob(const ssh_key_t * key,
                               ssh_string_t * *pblob)
{
    ssh_string_t * blob;
 ...
 */
int ssh_pki_export_pubkey_base64(const ssh_key_t * key,
                                 char **b64_key)
{
...
int ssh_pki_export_pubkey_file(const ssh_key_t * key,
                               const char *filename)
{
    char key_buf[4096];
    char host[256];
...
int ssh_pki_export_pubkey_rsa1(const ssh_key_t * key,
                               const char *host,
                               char *rsa1,
                               size_t rsa1_len)
{
...
int ssh_pki_import_signature_blob(const ssh_string_t * sig_blob,
...
int ssh_pki_signature_verify_blob(ssh_session_t * session,
                                  ssh_string_t * sig_blob,
                                  const ssh_key_t * key,
...
ssh_string_t * ssh_pki_do_sign(ssh_session_t * session,
                           ssh_buffer_t * sigbuf,
...
#ifndef _WIN32
ssh_string_t * ssh_pki_do_sign_agent(ssh_session_t * session,
                                 ssh_buffer_t *buf,
                                 const ssh_key_t * pubkey) {
    ssh_crypto_t *crypto;
    ssh_string_t * session_id;
    ssh_string_t * sig_blob;
    ssh_buffer_t * sig_buf;
    int rc;
...
#ifdef WITH_SERVER
ssh_string_t * ssh_srv_pki_do_sign_sessionid(ssh_session_t * session,
                                         const ssh_key_t * privkey)
{
    ssh_crypto_t *crypto;
    unsigned char hash[SHA_DIGEST_LEN] = {0};
