...
int ssh_socket_pollcallback(struct ssh_poll_handle_struct *p, socket_t fd, int revents, void *v_s){
	...
	if(revents & POLLOUT){
		...
		if(s->state == SSH_SOCKET_CONNECTING){
			SSH_INFO(SSH_LOG_PACKET,"Received POLLOUT in connecting state");
...
static int ssh_socket_unbuffered_write(ssh_socket s, const void *buffer,
  ...
  if(s->poll_out){
      SSH_INFO(SSH_LOG_PACKET, "Enabling POLLOUT for socket");
...
int ssh_socket_nonblocking_flush(ssh_socket s) {
  ssh_session_t * session = s->session;
  uint32_t len;
  int w;
...
int ssh_socket_connect(ssh_socket s, const char *host, int port, const char *bind_addr){
	...
	SSH_INFO(SSH_LOG_PROTOCOL,"Nonblocking connection socket: %d",fd);
	if(fd == SSH_INVALID_SOCKET)
		return SSH_ERROR;
	ssh_socket_set_fd(s,fd);
...
int ssh_socket_connect_proxycommand(ssh_socket s, const char *command){
  ...
  SSH_INFO(SSH_LOG_PROTOCOL,"Executing proxycommand '%s'",command);
  pid = fork();
  if(pid == 0){
  ...
  close(in_pipe[1]);
  close(out_pipe[0]);
  SSH_INFO(SSH_LOG_PROTOCOL,"ProxyCommand connection pipe: [%d,%d]",in_pipe[0],out_pipe[1]);
