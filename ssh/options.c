 ...
 */
int ssh_options_copy(ssh_session_t * src, ssh_session_t * *dest) {
    ssh_session_t * new;
    int i;
...
int ssh_options_set_algo(ssh_session_t * session, int algo,
    const char *list) {
 ...
 */
int ssh_options_set(ssh_session_t * session, enum ssh_options_e type,
    const void *value) {
    const char *v;
    char *p, *q;
    long int i;
 ...
 *
 */
int ssh_options_get_port(ssh_session_t * session, unsigned int* port_target) {
    if (session == NULL) {
        return -1;
    }
 ...
 */
int ssh_options_get(ssh_session_t * session, enum ssh_options_e type, char** value)
{
    char* src = NULL;
    ...
    switch(type)
    {
        ...
        case SSH_OPTIONS_IDENTITY: {
            ssh_iterator_t *it = ssh_list_get_iterator(session->opts.identity);
            if (it == NULL) {
                return SSH_ERROR;
            }
 ...
 * @see ssh_session_new()
 */
int ssh_options_getopt(ssh_session_t * session, int *argcptr, char **argv) {
  char *user = NULL;
  char *cipher = NULL;
  char *identity = NULL;
  char *port = NULL;
 ...
 */
int ssh_options_parse_config(ssh_session_t * session, const char *filename) {
...
int ssh_options_apply(ssh_session_t * session) {
    ssh_iterator_t *it;
    char *tmp;
    int rc;
 ...
 * @{
 */
static int ssh_bind_options_set_algo(ssh_bind_t * sshbind, int algo,
    const char *list) {
  if (!verify_existing_algo(algo, list)) {
 ...
 * @return              0 on success, < 0 on error.
 */
int ssh_bind_options_set(ssh_bind_t * sshbind, enum ssh_bind_options_e type,
    const void *value) {
  char *p, *q;
  int i;
