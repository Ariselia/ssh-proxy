...
static int dh_handshake_server(ssh_session_t * session);
...
static int server_set_kex(ssh_session_t * session) {
  ssh_kex_t *server = &session->next_crypto->server_kex;
  int i, j, rc;
  const char *wanted;
  char hostkeys[64] = {0};
 ...
 *        key exchange
 **/
static int ssh_server_kexdh_init(ssh_session_t * session, ssh_buffer_t * packet){
    ssh_string_t * e;
    e = buffer_get_ssh_string(packet);
	if (e == NULL) {
	#if 0
    #if 1
	  // WANGFENG: SSH2_MSG_KEX_DH_GEX_REQUEST_OLD ???
	  if(buffer_get_len(packet) < 16) {
	  	session->datafellows |= SSH_OLD_DHGEX;
	  	ssh_set_error(session, SSH_FATAL, "goto SSH2_MSG_KEX_DH_GEX_REQUEST_OLD");
		buffer_reinit(packet);
		return SSH_OK;
	  }
	#else
	  uint32_t en = 0;
	  //buffer_get_u32(packet, &en);
	  e = ssh_string_new(4);
	  buffer_get_data(packet, ssh_string_data(e), 4);
	#endif
	#endif
	  if(e == NULL) {
...
SSH_PACKET_CALLBACK(ssh_packet_kexdh_init){
  ...
  SSH_INFO(SSH_LOG_PACKET,"Received SSH_MSG_KEXDH_INIT");
  if(session->dh_handshake_state != DH_STATE_INIT){
    SSH_INFO(SSH_LOG_RARE,"Invalid state for SSH_MSG_KEXDH_INIT");
    goto error;
  }
...
int ssh_get_key_params(ssh_session_t * session, ssh_key_t * *privkey){
    ssh_key_t * pubkey;
    ssh_string_t * pubkey_blob;
    int rc;
...
static int dh_handshake_server(ssh_session_t * session) {
  ...
  SSH_INFO(SSH_LOG_PACKET, "SSH_MSG_NEWKEYS sent");
 ...
 * connection.
 */
static void ssh_server_connection_callback(ssh_session_t * session)
{
	int ssh1, ssh2;
	...
	switch(session->session_state)
	{
		...
		case SSH_SESSION_STATE_BANNER_RECEIVED:
		  	...
		  	SSH_INFO(SSH_LOG_RARE, "SSH client banner: %s", session->clientbanner);
		...
		case SSH_SESSION_STATE_DH:
			if(session->dh_handshake_state == DH_STATE_FINISHED)
			{
				...
				// WANGFENG: key exchange finished.
...
static int callback_receive_banner(const void *data, size_t len, void *user) {
    ...
    ssh_session_t * session = (ssh_session_t *) user;
    char *str = NULL;
    size_t i;
    int ret=0;
    ...
    for (i = 0; i < len; i++) {
        ...
        if (buffer[i] == '\n') {
            ...
            SSH_INFO(SSH_LOG_PACKET, "Received banner: %s", str);
 ...
 */
void ssh_set_auth_methods(ssh_session_t * session, int auth_methods){
	/* accept only methods in range */
	session->auth_methods = auth_methods & 0x3f;
}

/* Do the banner and key exchange */
int ssh_handle_key_exchange(ssh_session_t * session) {
    int rc = SSH_OK;
...
if(!session->proxy) {
    ...
    SSH_INFO(SSH_LOG_PACKET, "<<<<<<<<<<<<<<<<<<<<<< ssh_handle_key_exchange: current state : %d>>>>>>>>>>>>>>>>>>>>",
 ...
 */
int ssh_auth_reply_default(ssh_session_t * session,int partial) {
  char methods_c[128] = {0};
  ssh_string_t * methods = NULL;
  int rc = SSH_ERROR;
...
static int ssh_message_channel_request_open_reply_default(ssh_message_t * msg) {
  SSH_INFO(SSH_LOG_FUNCTIONS, "Refusing a channel");
...
static int ssh_message_channel_request_reply_default(ssh_message_t * msg) {
  uint32_t channel;
...
static int ssh_message_service_request_reply_default(ssh_message_t * msg) {
  /* The only return code accepted by specifications are success or disconnect */
  return ssh_message_service_reply_success(msg);
}

int ssh_message_service_reply_success(ssh_message_t * msg) {
  ssh_string_t *service;
...
int ssh_message_global_request_reply_success(ssh_message_t * msg, uint16_t bound_port) {
    SSH_INFO(SSH_LOG_FUNCTIONS, "Accepting a global request");
...
static int ssh_message_global_request_reply_default(ssh_message_t * msg) {
    SSH_INFO(SSH_LOG_FUNCTIONS, "Refusing a global request");
...
int ssh_message_reply_default(ssh_message_t * msg) {
  if (msg == NULL) {
    return -1;
  }
...
const char *ssh_message_service_service(ssh_message_t * msg){
  if (msg == NULL) {
    return NULL;
  }
...
const char *ssh_message_auth_user(ssh_message_t * msg) {
  if (msg == NULL) {
    return NULL;
  }
...
const char *ssh_message_auth_password(ssh_message_t * msg){
  if (msg == NULL) {
    return NULL;
  }
...
ssh_key_t * ssh_message_auth_pubkey(ssh_message_t * msg) {
  if (msg == NULL) {
    return NULL;
  }
...
ssh_public_key ssh_message_auth_publickey(ssh_message_t * msg){
  if (msg == NULL) {
    return NULL;
  }
...
enum ssh_publickey_state_e ssh_message_auth_publickey_state(ssh_message_t * msg){
	if (msg == NULL) {
	    return -1;
	  }
...
int ssh_message_auth_kbdint_is_response(ssh_message_t * msg) {
  if (msg == NULL) {
    return -1;
  }
...
int ssh_message_auth_set_methods(ssh_message_t * msg, int methods) {
...
int ssh_message_auth_interactive_request(ssh_message_t * msg, const char *name,
  ...
  if (msg->session->kbdint == NULL) {
    SSH_INFO(SSH_LOG_PROTOCOL, "Warning: Got a "
...
int ssh_auth_reply_success(ssh_session_t * session, int partial) {
  int r;
  ...
  //ssh_log(session, "\"SSH2_MSG_USERAUTH_SUCCESS(52)\"");
  if(session->current_crypto && session->current_crypto->delayed_compress_out){
      SSH_INFO(SSH_LOG_PROTOCOL,"Enabling delayed compression OUT");
  	session->current_crypto->do_compress_out=1;
  }
  if(session->current_crypto && session->current_crypto->delayed_compress_in){
      SSH_INFO(SSH_LOG_PROTOCOL,"Enabling delayed compression IN");
  	session->current_crypto->do_compress_in=1;
  }
  return r;
}

int ssh_message_auth_reply_success(ssh_message_t * msg, int partial) {
	if(msg == NULL) {
		return SSH_ERROR;
	}
...
int ssh_message_auth_reply_pk_ok(ssh_message_t * msg, ssh_string_t * algo, ssh_string_t * pubkey) {
  if (msg == NULL) {
    return SSH_ERROR;
  }
...
int ssh_message_auth_reply_pk_ok_simple(ssh_message_t * msg) {
    ssh_string_t * algo;
    ssh_string_t * pubkey_blob = NULL;
    int ret;
...
const char *ssh_message_channel_request_open_originator(ssh_message_t * msg){
    return msg->channel_request_open.originator;
}

int ssh_message_channel_request_open_originator_port(ssh_message_t * msg){
    return msg->channel_request_open.originator_port;
}

const char *ssh_message_channel_request_open_destination(ssh_message_t * msg){
    return msg->channel_request_open.destination;
}

int ssh_message_channel_request_open_destination_port(ssh_message_t * msg){
    return msg->channel_request_open.destination_port;
}

ssh_channel_t * ssh_message_channel_request_channel(ssh_message_t * msg){
    return msg->channel_request.channel;
}

const char *ssh_message_channel_request_pty_term(ssh_message_t * msg){
    return msg->channel_request.TERM;
}

int ssh_message_channel_request_pty_width(ssh_message_t * msg){
    return msg->channel_request.width;
}

int ssh_message_channel_request_pty_height(ssh_message_t * msg){
    return msg->channel_request.height;
}

int ssh_message_channel_request_pty_pxwidth(ssh_message_t * msg){
    return msg->channel_request.pxwidth;
}

int ssh_message_channel_request_pty_pxheight(ssh_message_t * msg){
    return msg->channel_request.pxheight;
}

const char *ssh_message_channel_request_env_name(ssh_message_t * msg){
    return msg->channel_request.var_name;
}

const char *ssh_message_channel_request_env_value(ssh_message_t * msg){
    return msg->channel_request.var_value;
}

const char *ssh_message_channel_request_command(ssh_message_t * msg){
    return msg->channel_request.command;
}

const char *ssh_message_channel_request_subsystem(ssh_message_t * msg){
    return msg->channel_request.subsystem;
}

int ssh_message_channel_request_x11_single_connection(ssh_message_t * msg){
    return msg->channel_request.x11_single_connection ? 1 : 0;
}

const char *ssh_message_channel_request_x11_auth_protocol(ssh_message_t * msg){
    return msg->channel_request.x11_auth_protocol;
}

const char *ssh_message_channel_request_x11_auth_cookie(ssh_message_t * msg){
    return msg->channel_request.x11_auth_cookie;
}

int ssh_message_channel_request_x11_screen_number(ssh_message_t * msg){
    return msg->channel_request.x11_screen_number;
}

const char *ssh_message_global_request_address(ssh_message_t * msg){
    return msg->global_request.bind_address;
}

int ssh_message_global_request_port(ssh_message_t * msg){
    return msg->global_request.bind_port;
}

/** @brief defines the ssh_message_t * callback
 ...
 */
void ssh_set_message_callback(ssh_session_t * session,
...
int ssh_execute_message_callbacks(ssh_session_t * session){
...
int ssh_send_keepalive(ssh_session_t * session)
{
  ssh_string_t *req;
  int rc;
  ...
  SSH_INFO(SSH_LOG_PACKET, "Sent a keepalive");
  return SSH_OK;
