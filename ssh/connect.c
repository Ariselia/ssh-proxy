...
static int getai(const char *host, int port, struct addrinfo **ai) {
  ...
  if (ssh_is_ipaddr(host)) {
    ...
    SSH_INFO(SSH_LOG_PACKET,"host %s matches an IP address",host);
...
static int ssh_connect_ai_timeout(ssh_session_t * session, const char *host,
  ...
  SSH_INFO(SSH_LOG_RARE, "Trying to connect to host: %s:%d with "
  ...
  /* s is connected ? */
  SSH_INFO(SSH_LOG_PACKET, "Socket connected with timeout\n");
 ...
 */
socket_t ssh_connect_host(ssh_session_t * session, const char *host,
  ...
  for (itr = ai; itr != NULL; itr = itr->ai_next){
    ...
    if (bind_addr) {
      ...
      SSH_INFO(SSH_LOG_PACKET, "Resolving %s\n", bind_addr);
 ...
 * @warning very ugly !!!
 */
socket_t ssh_connect_host_nonblocking(ssh_session_t * session, const char *host,
 ...
 * @see select(2)
 */
int ssh_select(ssh_channel_t * *channels, ssh_channel_t * *outchannels, socket_t maxfd,
  ...
  ssh_event_t * event = ssh_event_new();
  int firstround=1;
