 ...
 */
ssh_poll_ctx ssh_poll_get_default_ctx(ssh_session_t * session){
 ...
 *         ssh_session_t * objects and socket fd which are going to be polled at the
 *         same time as the event context. You would need a single event context
 *         per thread.
 * 
 * @return  The ssh_event_t * object on success, NULL on failure.
 */
ssh_event_t * ssh_event_new(void) {
    ssh_event_t * event;
 ...
 */
int ssh_event_add_fd(ssh_event_t * event, socket_t fd, short events,
 ...
 * when used in blocking mode.
 *
 * @param event     The ssh_event_t * object
 * @param session   The session to add to the event.
 *
 * @returns SSH_OK      on success
 *          SSH_ERROR   on failure
 */
int ssh_event_add_session(ssh_event_t * event, ssh_session_t * session) {
    unsigned int i;
    ssh_poll_handle p;
#ifdef WITH_SERVER
    ssh_iterator_t *iterator;
#endif
    ...
    while(iterator != NULL) {
        if((ssh_session_t *)iterator->data == session) {
 ...
 *         This function should be called once within the programs main loop.
 *
 * @param  event        The ssh_event_t * object to poll.
 ...
 *          SSH_ERROR   Error happened during the poll.
 */
int ssh_event_dopoll(ssh_event_t * event, int timeout) {
    int rc;
 ...
 *          SSH_ERROR   on failure
 */
int ssh_event_remove_fd(ssh_event_t * event, socket_t fd) {
 ...
 *          SSH_ERROR   on failure
 */
int ssh_event_remove_session(ssh_event_t * event, ssh_session_t * session) {
    ssh_poll_handle p;
    register size_t i, used;
    int rc = SSH_ERROR;
#ifdef WITH_SERVER
