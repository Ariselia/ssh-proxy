...
/**
 * @defgroup ssh_scp The SSH scp functions
 * @ingroup ssh
 *
 ...
 *
 * @returns             A ssh_scp_t * handle, NULL if the creation was impossible.
 */
ssh_scp_t * ssh_scp_new(ssh_session_t * session, int mode, const char *location){
  	ssh_scp_t *scp = malloc(sizeof(ssh_scp_t));
  	if(scp == NULL){
    	ssh_set_error(session,SSH_FATAL,"Error allocating memory for ssh_scp_t *");
    	return NULL;
  	}
  	ZERO_STRUCTP(scp);
 ...
 * @see ssh_scp_new()
 */
int ssh_scp_init(ssh_scp_t *scp)
{
  	int r;
  	...
  	SSH_INFO(SSH_LOG_PROTOCOL,"Initializing scp session %s %son location '%s'",
 ...
 * @see ssh_scp_init()
 */
int ssh_scp_close(ssh_scp_t *scp)
{
  	char buffer[128];
  	int err;
  	if(scp == NULL) {
 ...
 * @see ssh_scp_new()
 */
void ssh_scp_free(ssh_scp_t * scp)
{
  	if(scp == NULL) {
      	return;
  	}
 ...
 */
int ssh_scp_push_directory(ssh_scp_t * scp, const char *dirname, int mode){
  char buffer[1024];
  int r;
  uint8_t code;
  char *dir;
 ...
 */
 int ssh_scp_leave_directory(ssh_scp_t * scp){
  	char buffer[] = "E\n";
  	int r;
  	uint8_t code;
  	if(scp == NULL) {
 ...
 */
int ssh_scp_push_file64(ssh_scp_t * scp, const char *filename, uint64_t size, int mode){
  char buffer[1024];
  int r;
  uint8_t code;
  char *file;
  ...
  perms=ssh_scp_string_mode(mode);
  SSH_INFO(SSH_LOG_PROTOCOL,"SCP pushing file %s, size %" PRIu64 " with permissions '%s'",file,size,perms);
 ...
 *                      error occured.
 */
int ssh_scp_push_file(ssh_scp_t * scp, const char *filename, size_t size, int mode){
...
int ssh_scp_response(ssh_scp_t * scp, char **response){
	...
	if(code == 1){
		...
		SSH_INFO(SSH_LOG_RARE,"SCP: Warning: status code 1 received: %s", msg);
		if(response)
			*response=strdup(msg);
		return 1;
	}
 ...
 */
int ssh_scp_write(ssh_scp_t * scp, const void *buffer, size_t len){
  int w;
  int r;
  uint8_t code;
  if(scp==NULL)
 ...
 */
int ssh_scp_pull_request(ssh_scp_t * scp){
  ...
  if(p!=NULL)
	  *p='\0';
  SSH_INFO(SSH_LOG_PROTOCOL,"Received SCP request: '%s'",buffer);
  switch(buffer[0]){
    case 'C':
      /* File */
    case 'D':
 ...
 */
int ssh_scp_deny_request(ssh_scp_t * scp, const char *reason){
 ...
 */
int ssh_scp_accept_request(ssh_scp_t * scp){
  char buffer[]={0x00};
  int err;
  if(scp==NULL)
      return SSH_ERROR;
 ...
 *                      while reading.
 */
int ssh_scp_read(ssh_scp_t * scp, void *buffer, size_t size){
  int r;
  int code;
  if(scp==NULL)
      return SSH_ERROR;
 ...
 *                      freed.
 */
const char *ssh_scp_request_get_filename(ssh_scp_t * scp){
  if(scp==NULL)
      return NULL;
 ...
 */
int ssh_scp_request_get_permissions(ssh_scp_t * scp){
  if(scp==NULL)
      return -1;
 ...
 */
size_t ssh_scp_request_get_size(ssh_scp_t * scp){
  if(scp==NULL)
      return 0;
 ...
 * @returns             The numeric size of the file being read.
 */
uint64_t ssh_scp_request_get_size64(ssh_scp_t * scp){
  if(scp==NULL)
      return 0;
  return scp->filelen;
}
 ...
 *                      not be freed.
 */
const char *ssh_scp_request_get_warning(ssh_scp_t * scp){
  if(scp==NULL)
      return NULL;
	return scp->warning;
}
