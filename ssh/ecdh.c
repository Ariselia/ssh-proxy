 ...
 */
int ssh_client_ecdh_init(ssh_session_t * session){
  EC_KEY *key;
...
static void ecdh_import_pubkey(ssh_session_t * session, ssh_string_t * pubkey_string) {
  session->next_crypto->server_pubkey = pubkey_string;
}

static int ecdh_build_k(ssh_session_t * session) {
 ...
 * a SSH_MSG_NEWKEYS
 */
int ssh_client_ecdh_reply(ssh_session_t * session, ssh_buffer_t * packet){
...
int ssh_server_ecdh_init(ssh_session_t * session, ssh_buffer_t * packet){
    /* ECDH keys */
    ...
    SSH_INFO(SSH_LOG_PROTOCOL, "SSH_MSG_KEXDH_REPLY sent");
    rc = packet_send(session);
    if (rc == SSH_ERROR) {
        return SSH_ERROR;
    }
