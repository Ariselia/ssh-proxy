...
static socket_t bind_socket(ssh_bind_t * sshbind, const char *hostname,
    int port) {
    char port_c[6];
    struct addrinfo *ai;
...
ssh_bind_t * ssh_bind_new2(int port) {
  ssh_bind_t * ptr;

  ptr = malloc(sizeof(ssh_bind_t));
  if (ptr == NULL) {
    return NULL;
  }
  ZERO_STRUCTP(ptr);
...
ssh_bind_t * ssh_bind_new(void) {
  return ssh_bind_new2(22);
}


static int ssh_bind_import_keys(ssh_bind_t * sshbind) {
  int rc;
...
int ssh_bind_listen(ssh_bind_t * sshbind) {
  const char *host;
  socket_t fd;
  int rc;
  ...
  if (sshbind->bindfd == SSH_INVALID_SOCKET) {
      ...
      SSH_INFO(SSH_LOG_INFO, "Using app-provided bind socket");
  }
  return 0;
}

int ssh_bind_set_callbacks(ssh_bind_t * sshbind, ssh_bind_callbacks callbacks,
    void *userdata){
  if (sshbind == NULL) {
    return SSH_ERROR;
  }
...
static int ssh_bind_poll_callback(ssh_poll_handle sshpoll,
  ...
  ssh_bind_t * sshbind=(ssh_bind_t *)user;
  (void)sshpoll;
  (void)fd;
 ...
 * @param sshbind the ssh_bind_t * object
 * @returns a ssh_poll handle suitable for operation
 */
ssh_poll_handle ssh_bind_get_poll(ssh_bind_t * sshbind){
  if(sshbind->poll)
    return sshbind->poll;
...
void ssh_bind_set_blocking(ssh_bind_t * sshbind, int blocking) {
  sshbind->blocking = blocking ? 1 : 0;
}

socket_t ssh_bind_get_fd(ssh_bind_t * sshbind) {
  return sshbind->bindfd;
}

void ssh_bind_set_fd(ssh_bind_t * sshbind, socket_t fd) {
  sshbind->bindfd = fd;
}

void ssh_bind_fd_toaccept(ssh_bind_t * sshbind) {
  sshbind->toaccept = 1;
}

void ssh_bind_free(ssh_bind_t * sshbind){
  int i;
...
int ssh_bind_accept_fd(ssh_bind_t * sshbind, ssh_session_t * session, socket_t fd){
    int i, rc;
     ...
     * where keys can be imported) on this ssh_bind_t * and are instead
...
int ssh_bind_accept(ssh_bind_t * sshbind, ssh_session_t * session) {
