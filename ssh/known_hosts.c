 ...
 *                      on error.
 */
static int check_public_key(ssh_session_t * session, char **tokens) {
  ssh_string_t * pubkey = session->current_crypto->server_pubkey;
  ssh_buffer_t * pubkey_buffer;
  char *pubkey_64;
 ...
 *      host table.
 */
int ssh_is_server_known(ssh_session_t * session) {
  FILE *file = NULL;
  char **tokens;
  char *host;
  char *hostport;
 ...
 */
int ssh_write_knownhost(ssh_session_t * session) {
    ssh_key_t * key;
    ssh_string_t * pubkey_s;
    char *b64_key;
    char buffer[4096] = {0};
    FILE *file;
    char *dir;
 ...
 *			NULL on error
 */
char **ssh_knownhosts_algorithms(ssh_session_t * session) {
  FILE *file = NULL;
  char **tokens;
  char *host;
  char *hostport;
  ...
  do {
    ...
    if (match) {
      /* We got a match. Now check the key type */
    	SSH_INFO(SSH_LOG_DEBUG, "server %s:%d has %s in known_hosts",
