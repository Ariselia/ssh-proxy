...
/* functions */
static int sftp_enqueue(sftp_session_t *session, sftp_message_t * msg);
static void sftp_message_free(sftp_message_t * msg);
static void sftp_set_error(sftp_session_t *sftp, int errnum);
static void status_msg_free(sftp_status_message_t *status);

static sftp_ext_t *sftp_ext_new(void) {
  sftp_ext_t *ext;
...
static void sftp_ext_free(sftp_ext_t *ext) {
  unsigned int i;
...
sftp_session_t *sftp_new(ssh_session_t * session){
  sftp_session_t *sftp;
...
sftp_session_t *sftp_new_channel(ssh_session_t *session, ssh_channel_t *channel)
{
  	sftp_session_t *sftp;
...
#ifdef WITH_SERVER
sftp_session_t *sftp_server_new(ssh_session_t *session, ssh_channel_t *chan){
...
int sftp_server_init(sftp_session_t *sftp)
{
  	ssh_session_t * session = sftp->session;
  	sftp_packet_t * packet = NULL;
  	...
  	SSH_INFO(SSH_LOG_PACKET, "Received SSH_FXP_INIT");
	
  	buffer_get_u32(packet->payload, &version);
  	version = ntohl(version);
  	SSH_INFO(SSH_LOG_PACKET, "Client version: %d", version);
  	...
  	SSH_INFO(SSH_LOG_RARE, "Server version sent");
...
int sftp_get_error(sftp_session_t *sftp) {
  	if (sftp == NULL) {
    	return -1;
  	}
...
static sftp_message_t * sftp_message_new(sftp_session_t *sftp){
  sftp_message_t * msg = NULL;
...
static sftp_message_t *sftp_get_message(sftp_packet_t *packet)
{
	sftp_session_t *sftp = packet->sftp;
  	sftp_message_t *msg = NULL;
...
static int sftp_read_and_dispatch(sftp_session_t *sftp)
{
	sftp_packet_t *packet = NULL;
	sftp_message_t *msg = NULL;
...
int sftp_init(sftp_session_t *sftp)
{
  	...
  	ssh_string_t * ext_name_s = NULL;
  	ssh_string_t * ext_data_s = NULL;
  	char *ext_name = NULL;
  	char *ext_data = NULL;
  	...
  	buffer_get_u32(packet->payload, &version);
  	version = ntohl(version);
  	SSH_INFO(SSH_LOG_RARE, "SFTP server version %d", version);
...
unsigned int sftp_extensions_get_count(sftp_session_t *sftp) {
...
const char *sftp_extensions_get_name(sftp_session_t *sftp, unsigned int idx) {
  if (sftp == NULL)
    return NULL;
...
const char *sftp_extensions_get_data(sftp_session_t *sftp, unsigned int idx) {
  if (sftp == NULL)
    return NULL;
  if (sftp->ext == NULL || sftp->ext->name == NULL) {
    ssh_set_error_invalid(sftp->session);
...
int sftp_extension_supported(sftp_session_t *sftp, const char *name,
    const char *data) {
  int i, n;
...
static sftp_request_queue_t *request_queue_new(sftp_message_t * msg) {
  sftp_request_queue_t *queue = NULL;
...
static void request_queue_free(sftp_request_queue_t *queue) {
  if (queue == NULL) {
    return;
  }
...
static int sftp_enqueue(sftp_session_t *sftp, sftp_message_t * msg) {
  sftp_request_queue_t *queue = NULL;
  sftp_request_queue_t *ptr;
 ...
 */
static sftp_message_t * sftp_dequeue(sftp_session_t *sftp, uint32_t id){
  sftp_request_queue_t *prev = NULL;
 ...
 */
static inline uint32_t sftp_get_new_id(sftp_session_t *session) {
  return ++session->id_counter;
}

static sftp_status_message_t *parse_status_msg(sftp_message_t * msg){
  sftp_status_message_t *status;
...
static sftp_file_t * parse_handle_msg(sftp_message_t * msg){
  sftp_file_t * file;
 ...
 */
static sftp_attributes_t *sftp_parse_attr_4(sftp_session_t *sftp, ssh_buffer_t * buf,
    int expectnames) {
  sftp_attributes_t *attr;
  ssh_string_t * owner = NULL;
  ssh_string_t * group = NULL;
  uint32_t flags = 0;
  int ok = 0;
  ...
  attr = malloc(sizeof(sftp_attributes_t));
  if (attr == NULL) {
    ssh_set_error_oom(sftp->session);
    return NULL;
  }
...
static sftp_attributes_t *sftp_parse_attr_3(sftp_session_t *sftp, ssh_buffer_t * buf,
    int expectname) {
  ssh_string_t * longname;
  ssh_string_t * name;
  sftp_attributes_t *attr;
  uint32_t flags = 0;
  int ok = 0;
  ...
  do {
    if (expectname) {
      ...
      SSH_INFO(SSH_LOG_RARE, "Name: %s", attr->name);
...
sftp_attributes_t *sftp_parse_attr(sftp_session_t *session, ssh_buffer_t * buf,
    int expectname) {
...
int sftp_server_version(sftp_session_t *sftp) {
  return sftp->server_version;
}

/* Get a single file attributes structure of a directory. */
sftp_attributes_t *sftp_readdir(sftp_session_t *sftp, sftp_dir_t *dir) {
  sftp_message_t * msg = NULL;
  sftp_status_message_t *status;
  sftp_attributes_t *attr;
  ssh_buffer_t * payload;
  ...
  SSH_INFO(SSH_LOG_RARE, "Count is %d", dir->count);
...
void sftp_attributes_free(sftp_attributes_t *file){
  if (file == NULL) {
    return;
  }
...
static int sftp_handle_close(sftp_session_t *sftp, ssh_string_t * handle) {
  sftp_status_message_t *status;
  sftp_message_t * msg = NULL;
  ssh_buffer_t * buffer = NULL;
  uint32_t id;
...
sftp_file_t * sftp_open(sftp_session_t *sftp, const char *file, int flags,
    mode_t mode) {
  sftp_message_t * msg = NULL;
  sftp_status_message_t *status;
  sftp_attributes_t attr;
  sftp_file_t * handle;
  ssh_string_t * filename;
  ssh_buffer_t * buffer;
  ...
  SSH_INFO(SSH_LOG_PACKET,"Opening file %s with sftp flags %x",file,sftp_flags);
  id = sftp_get_new_id(sftp);
  if (buffer_add_u32(buffer, id) < 0 ||
...
void sftp_file_set_nonblocking(sftp_file_t * handle){
    handle->nonblocking=1;
}
void sftp_file_set_blocking(sftp_file_t * handle){
    handle->nonblocking=0;
}

/* Read from a file using an opened sftp file handle. */
ssize_t sftp_read(sftp_file_t * handle, void *buf, size_t count) {
  sftp_session_t *sftp = handle->sftp;
  sftp_message_t * msg = NULL;
  sftp_status_message_t *status;
  ssh_string_t * datastring;
  ssh_buffer_t * buffer;
  int id;
...
int sftp_async_read_begin(sftp_file_t * file, uint32_t len){
...
int sftp_async_read(sftp_file_t * file, void *data, uint32_t size, uint32_t id){
  sftp_session_t *sftp;
  sftp_message_t * msg = NULL;
  sftp_status_message_t *status;
  ssh_string_t * datastring;
...
ssize_t sftp_write(sftp_file_t * file, const void *buf, size_t count) {
  sftp_session_t *sftp = file->sftp;
  sftp_message_t * msg = NULL;
  sftp_status_message_t *status;
  ssh_string_t * datastring;
...
int sftp_seek(sftp_file_t * file, uint32_t new_offset) {
  if (file == NULL) {
    return -1;
  }
...
int sftp_seek64(sftp_file_t * file, uint64_t new_offset) {
  if (file == NULL) {
    return -1;
  }
...
unsigned long sftp_tell(sftp_file_t * file) {
  return (unsigned long)file->offset;
}
/* Report current byte position in file. */
uint64_t sftp_tell64(sftp_file_t * file) {
...
int sftp_unlink(sftp_session_t *sftp, const char *file) {
  sftp_status_message_t *status = NULL;
  sftp_message_t * msg = NULL;
  ssh_string_t * filename;
  ssh_buffer_t * buffer;
  uint32_t id;
...
/* code written by Nick */
int sftp_rmdir(sftp_session_t *sftp, const char *directory) {
  sftp_status_message_t *status = NULL;
  sftp_message_t * msg = NULL;
  ssh_string_t * filename;
  ssh_buffer_t * buffer;
...
/* Code written by Nick */
int sftp_mkdir(sftp_session_t *sftp, const char *directory, mode_t mode) {
  sftp_status_message_t *status = NULL;
  sftp_message_t * msg = NULL;
  sftp_attributes_t *errno_attr = NULL;
  sftp_attributes_t attr;
  ssh_buffer_t * buffer;
  ssh_string_t * path;
  uint32_t id;
...
/* code written by nick */
int sftp_rename(sftp_session_t *sftp, const char *original, const char *newname) {
  sftp_status_message_t *status = NULL;
  sftp_message_t * msg = NULL;
  ssh_buffer_t * buffer;
  ssh_string_t * oldpath;
  ssh_string_t * newpath;
  uint32_t id;
...
int sftp_setstat(sftp_session_t *sftp, const char *file, sftp_attributes_t *attr) {
  uint32_t id;
  ssh_buffer_t * buffer;
  ssh_string_t * path;
  sftp_message_t * msg = NULL;
...
int sftp_chown(sftp_session_t *sftp, const char *file, uid_t owner, gid_t group) {
	sftp_attributes_t attr;
  ZERO_STRUCT(attr);
...
int sftp_chmod(sftp_session_t *sftp, const char *file, mode_t mode) {
	sftp_attributes_t attr;
  ZERO_STRUCT(attr);
  attr.permissions = mode;
  attr.flags = SSH_FILEXFER_ATTR_PERMISSIONS;
...
int sftp_utimes(sftp_session_t *sftp, const char *file,
...
char *sftp_readlink(sftp_session_t *sftp, const char *path) {
  sftp_status_message_t *status = NULL;
  sftp_message_t * msg = NULL;
  ssh_string_t * path_s = NULL;
  ssh_string_t * link_s = NULL;
  ssh_buffer_t * buffer;
  char *lnk;
  uint32_t ignored;
  uint32_t id;
...
static sftp_statvfs_t sftp_parse_statvfs(sftp_session_t *sftp, ssh_buffer_t * buf) {
...
sftp_statvfs_t sftp_statvfs(sftp_session_t *sftp, const char *path) {
  sftp_status_message_t *status = NULL;
  sftp_message_t * msg = NULL;
  ssh_string_t * pathstr;
  ssh_string_t * ext;
  ssh_buffer_t * buffer;
  uint32_t id;
...
sftp_statvfs_t sftp_fstatvfs(sftp_file_t * file) {
  sftp_status_message_t *status = NULL;
  sftp_message_t * msg = NULL;
  sftp_session_t *sftp;
  ssh_string_t * ext;
...
static sftp_attributes_t *sftp_xstat(sftp_session_t *sftp, const char *path,
    int param) {
  sftp_status_message_t *status = NULL;
  sftp_message_t * msg = NULL;
  ssh_string_t * pathstr;
  ...
  if (msg->packet_type == SSH_FXP_ATTRS) {
    sftp_attributes_t *attr = sftp_parse_attr(sftp, msg->payload, 0);
    sftp_message_free(msg);
...
sftp_attributes_t *sftp_stat(sftp_session_t *session, const char *path) {
  return sftp_xstat(session, path, SSH_FXP_STAT);
}

sftp_attributes_t *sftp_lstat(sftp_session_t *session, const char *path) {
