...
ssh_list_t *ssh_list_new(void) {
  ssh_list_t *ret=malloc(sizeof(ssh_list_t));
  if(!ret)
    return NULL;
...
static ssh_iterator_t *ssh_iterator_new(const void *data){
  ssh_iterator_t *iterator=malloc(sizeof(ssh_iterator_t));
  if(!iterator)
    return NULL;
  iterator->next=NULL;
  iterator->data=data;
...
int ssh_list_append(ssh_list_t *list,const void *data){
  ssh_iterator_t *iterator=ssh_iterator_new(data);
  if(!iterator)
    return SSH_ERROR;
  if(!list->end){
    /* list is empty */
...
int ssh_list_prepend(ssh_list_t *list, const void *data){
  ssh_iterator_t *it = ssh_iterator_new(data);
...
const void *_ssh_list_pop_head(ssh_list_t *list){
  ssh_iterator_t *iterator=list->root;
  const void *data;
  if(!list->root)
    return NULL;
  data=iterator->data;
...
int ssh_analyze_banner(ssh_session_t * session, int server, int *ssh1, int *ssh2) {
  ...
  SSH_INFO(SSH_LOG_RARE, "Analyzing banner: %s", banner);
