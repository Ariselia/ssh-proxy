...
static void socket_callback_connected(int code, int errno_code, void *user){
	...
	SSH_INFO(SSH_LOG_RARE,"Socket connection callback: %d (%d)",code, errno_code);
 ...
 */
int ssh_send_banner(ssh_session_t * session, int server) {
...
if(!session->proxy) {
  ...
  if(session->data_send) {
  	session->data_send(session, 0, buffer, strlen(buffer));
  }
}
  err = SSH_OK;
end:
 ...
 * completed
 */
static int dh_handshake(ssh_session_t * session) {
 ...
 */
int ssh_service_request(ssh_session_t * session, const char *service) {
  ssh_string_t * service_s = NULL;
  int rc=SSH_ERROR;
  ...
  //ssh_log(session, "\"SSH2_MSG_SERVICE_REQUEST(5)\"");
  rc = SSH_OK;
pending:
if(!session->proxy) {
 ...
 * connection.
 */
static void ssh_client_connection_callback(ssh_session_t * session){
	int ssh1,ssh2;
...
if(!session->proxy) {
  	...
  	SSH_INFO(SSH_LOG_RARE, "SSH_API %s, using threading %s", ssh_copyright(), ssh_threads_get_type());
  	// WANGFENG: client callback
...
if(!session->proxy) {
  	...
  	session->alive = 1;
  	SSH_INFO(SSH_LOG_PROTOCOL,"Socket connecting, now waiting for the callbacks to work");
pending:
if(!session->proxy)
{
  	...
  	if(ssh_is_blocking(session)) {
      	...
      	SSH_INFO(SSH_LOG_PACKET,"ssh_connect: Actual timeout : %d", timeout);
  	...
  	SSH_INFO(SSH_LOG_PACKET,"ssh_connect: Actual state : %d",session->session_state);
 ...
 */
char *ssh_get_issue_banner(ssh_session_t * session) {
 ...
 * @endcode
 */
int ssh_get_openssh_version(ssh_session_t * session) {
  	if (session == NULL) {
    	return 0;
  	}
...
error:
  	...
  	if(session->ssh_message_list)
	{
    	...
    	while((msg = ssh_list_pop_head(ssh_message_t * ,session->ssh_message_list))
        	!= NULL)
        {
...
const char *ssh_copyright(void) {
    return SSH_STRINGIFY(LIBSSH_VERSION) " (c) 2003-2014 MyMMSC Software Foundation (MSF)";
}
/** @} */
