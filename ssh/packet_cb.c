...
SSH_PACKET_CALLBACK(ssh_packet_disconnect_callback){
  ...
  SSH_INFO(SSH_LOG_PACKET, "Received SSH_MSG_DISCONNECT %d:%s",code,
...
SSH_PACKET_CALLBACK(ssh_packet_ignore_callback){
	...
	(void)type;
	(void)packet;
	SSH_INFO(SSH_LOG_PROTOCOL,"Received %s packet",type==SSH2_MSG_IGNORE ? "SSH_MSG_IGNORE" : "SSH_MSG_DEBUG");
	/* TODO: handle a graceful disconnect */
	return SSH_PACKET_USED;
}
...
SSH_PACKET_CALLBACK(ssh_packet_dh_reply){
  ...
  (void)type;
  (void)user;
  SSH_INFO(SSH_LOG_PROTOCOL,"Received SSH_KEXDH_REPLY");
...
SSH_PACKET_CALLBACK(ssh_packet_newkeys){
  ...
  (void)user;
  (void)type;
  SSH_INFO(SSH_LOG_PROTOCOL, "Received SSH_MSG_NEWKEYS");
  if(session->session_state!= SSH_SESSION_STATE_DH &&
		session->dh_handshake_state != DH_STATE_NEWKEYS_SENT){
  ...
  if(session->server){
    ...
    SSH_INFO(SSH_LOG_PROTOCOL,"Signature verified and valid");
...
SSH_PACKET_CALLBACK(ssh_packet_service_accept){
	...
	// WANGFENG: proxy
	session->command = SSH2_MSG_SERVICE_ACCEPT;
	return SSH_PACKET_USED;
}
