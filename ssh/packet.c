...
int ssh_packet_socket_callback(const void *data, size_t receivedlen, void *user)
{
	...
	//trace_err("data length = [%d]", receivedlen);
	//ssh_print_hexa("Hex: ", data, receivedlen);
    ...
    switch(session->packet_state) {
        ...
        case PACKET_STATE_SIZEREAD:
            ...
            /* Execute callbacks */
            ssh_packet_process(session, session->in_packet.type, "UNKNOWN");
...
void ssh_packet_register_socket_callback(ssh_session_t * session, ssh_socket s){
 ...
 */
void ssh_packet_set_callbacks(ssh_session_t * session, ssh_packet_callbacks callbacks){
 ...
 */
void ssh_packet_set_default_callbacks(ssh_session_t * session){
#ifdef WITH_SSH1
 ...
 */
void ssh_packet_process(ssh_session_t * session, uint8_t type, const char *name){
	ssh_iterator_t *i;
	int r = SSH_PACKET_NOT_USED;
	ssh_packet_callbacks cb;
	
	SSH_INFO(SSH_LOG_PACKET, "Dispatching handler for packet type %d[%s]", type, name);
	if(session->packet_callbacks == NULL){
		SSH_INFO(SSH_LOG_RARE,"%s(%d), Packet callback is not initialized !", name, type);
		return;
	}
	...
	while(i != NULL){
		...
		if(r == SSH_PACKET_USED) {
			...
			//ssh_log(session, "\"%s(%d)\"", name, type);
			break;
		}
	}
	if(r == SSH_PACKET_NOT_USED){
		SSH_INFO(SSH_LOG_RARE,"Couldn't do anything with packet type %d", type);
 ...
 */
int ssh_packet_send_unimplemented(ssh_session_t * session, uint32_t seqnum){
  int r;
...
SSH_PACKET_CALLBACK(ssh_packet_unimplemented){
  ...
  seq = ntohl(seq);
  SSH_INFO(SSH_LOG_RARE, "Received SSH_MSG_UNIMPLEMENTED (sequence number %d)", seq);
 ...
 */
int ssh_packet_parse_type(ssh_session_t * session) {
  memset(&session->in_packet, 0, sizeof(PACKET));
  if(session->in_buffer == NULL) {
    return SSH_ERROR;
  }
 ...
 * socket buffer
 */
static int ssh_packet_write(ssh_session_t *session, uint8_t command) {
	int rc = SSH_ERROR;
	// WANGFENG: proxy
	if(session->data_send) {
		rc = session->data_send(session, command, buffer_get_rest(session->out_buffer),
			buffer_get_rest_len(session->out_buffer));
	} else {
		rc = ssh_socket_write(session->socket, buffer_get_rest(session->out_buffer),
			buffer_get_rest_len(session->out_buffer));
	}
	return rc;
}

static int packet_send2(ssh_session_t *session) {
	uint8_t command = 2; // default to 2
	unsigned int blocksize = (session->current_crypto ?
	...
	// WANGFENG: proxy
	command = session->out_buffer->data[0];
	...
	rc = ssh_packet_write(session, command);
	session->send_seq++;
...
int packet_send(ssh_session_t * session) {
#ifdef WITH_SSH1
	if (session->version == 1) {
