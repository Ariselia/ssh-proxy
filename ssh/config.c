...
static int ssh_config_parse_line(ssh_session_t * session, const char *line,
  ...
  switch (opcode) {
    ...
    case SOC_UNSUPPORTED:
      SSH_INFO(SSH_LOG_RARE, "Unsupported option: %s, line: %d\n",
              keyword, count);
      break;
    default:
...
int ssh_config_parse_file(ssh_session_t * session, const char *filename) {
  char line[1024] = {0};
  ...
  SSH_INFO(SSH_LOG_RARE, "Reading configuration data from %s", filename);
