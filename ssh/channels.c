...
static ssh_channel_t *channel_from_msg(ssh_session_t *session, ssh_buffer_t *packet);
 ...
 */
uint32_t ssh_channel_new_id(ssh_session_t *session)
{
...
SSH_PACKET_CALLBACK(ssh_packet_channel_open_conf){
    ...
    SSH_INFO(SSH_LOG_PACKET,"Received SSH2_MSG_CHANNEL_OPEN_CONFIRMATION");
    ...
    channel->flags &= ~SSH_CHANNEL_FLAG_NOT_BOUND;
    // WANGFENG: server>proxy flag
    session->command = SSH2_MSG_CHANNEL_OPEN_CONFIRMATION;
    return SSH_PACKET_USED;
}
...
SSH_PACKET_CALLBACK(ssh_packet_channel_open_fail)
{
    ...
    if(channel == NULL){
        SSH_INFO(SSH_LOG_RARE,"Invalid channel in packet");
        return SSH_PACKET_USED;
    }
    ...
    ssh_set_error(session, SSH_REQUEST_DENIED,
            ...
            channel->local_channel, (long unsigned int) ntohl(code), error);
    SAFE_FREE(error);
...
{
    int iRet = 1;
    ssh_channel_t *channel = (ssh_channel_t *)c;
 ...
 */
static int channel_open(ssh_channel_t *channel, const char *type_c, int window,
    int maxpacket, ssh_buffer_t *payload)
{
...
{
    ssh_iterator_t *it = NULL;
    ssh_channel_t *channel = NULL;
    
    for (it = ssh_list_get_iterator(session->channels); it != NULL ; it=it->next) {
        channel = ssh_iterator_value(ssh_channel_t *, it);
        if (channel == NULL) {
            continue;
        }
 ...
 */
static int grow_window(ssh_session_t * session, ssh_channel_t * channel, int minimumsize)
{
    ...
    SSH_INFO(SSH_LOG_PROTOCOL,
            ...
            channel->remote_channel,
            new_window);
    //ssh_log(session, "\"SSH2_MSG_CHANNEL_WINDOW_ADJUST(93)\"");
 ...
 * @internal
 *
 * @brief Parse a channel-related packet to resolve it to a ssh_channel_t *.
 *
 ...
 * @returns             The related ssh_channel_t *, or NULL if the channel is
...
SSH_PACKET_CALLBACK(channel_rcv_change_window)
{
    ...
    channel = channel_from_msg(session,packet);
    if (channel == NULL) {
        SSH_INFO(SSH_LOG_FUNCTIONS, "%s", ssh_get_error(session));
    }
...
SSH_PACKET_CALLBACK(channel_rcv_data)
{
    ...
    if (str == NULL) {
        SSH_INFO(SSH_LOG_PACKET, "Invalid data packet!");
        return SSH_PACKET_USED;
    }
    ...
    if (len > channel->local_window) {
        SSH_INFO(SSH_LOG_RARE,
                "Data packet too big for our window(%" PRIdS " vs %d)",
                len, channel->local_window);
    }

    if (channel_default_bufferize(channel, ssh_string_data(str), len, is_stderr) < 0)
    {
        ssh_string_free(str);
        return SSH_PACKET_USED;
    }
...
SSH_PACKET_CALLBACK(channel_rcv_close)
{
	...
	if(ssh_callbacks_exists(channel->callbacks, channel_close_function))
    {
		channel->callbacks->channel_close_function(channel->session,
			channel, channel->callbacks->userdata);
	}
...
SSH_PACKET_CALLBACK(channel_rcv_request)
{
	...
	if (request_s == NULL) {
		SSH_INFO(SSH_LOG_PACKET, "Invalid MSG_CHANNEL_REQUEST");
		return SSH_PACKET_USED;
	}
	...
	if (strcmp(request,"exit-status") == 0) {
		...
		SSH_INFO(SSH_LOG_PACKET, "received exit-status %d", channel->exit_status);
    ...
    {
		ssh_string_t *signal_str = NULL;
        char *sig = NULL;

		SAFE_FREE(request);
		SSH_INFO(SSH_LOG_PACKET, "received signal");
    ...
    {
        SAFE_FREE(request);
	    SSH_INFO(SSH_LOG_PROTOCOL,"Responding to Openssh's keepalive");
    ...
    {
        SAFE_FREE(request);
        SSH_INFO(SSH_LOG_PROTOCOL, "Received an auth-agent-req request");
        if(ssh_callbacks_exists(channel->callbacks, channel_auth_agent_req_function))
        {
            channel->callbacks->channel_auth_agent_req_function(channel->session,
                    channel,
                    channel->callbacks->userdata);
        }
	 ...
	 */
	 // WANGFENG: SSH2_MSG_CHANNEL_REQUEST goto
	ssh_message_handle_channel_request(session, channel, packet, request, status);
#else
    SSH_INFO(SSH_LOG_WARNING, "Unhandled channel request %s", request);
#endif
 ...
 * @see ssh_channel_open_forward()
 */
int ssh_channel_open_auth_agent(ssh_channel_t * channel)
{
    if(channel == NULL) {
        return SSH_ERROR;
    }
 ...
 */
int ssh_channel_open_forward(ssh_channel_t *channel, const char *remotehost,
...
{
    ssh_session_t * session = (ssh_session_t *)s;
...
int channel_write_common(ssh_channel_t * channel, const void *data,
...
if(!session->proxy) {
	...
	if(!session->proxy) {
        ...
        if (is_stderr && buffer_add_u32(session->out_buffer, htonl(SSH2_EXTENDED_DATA_STDERR)) < 0)
        {
            ssh_set_error_oom(session);
            goto error;
        }
...
uint32_t ssh_channel_window_size(ssh_channel_t * channel)
{
 ...
 */
int ssh_channel_write(ssh_channel_t *channel, const void *data, uint32_t len)
{
 ...
 */
int ssh_channel_is_closed(ssh_channel_t * channel) {
    if(channel == NULL) {
        return SSH_ERROR;
    }
    return (channel->state != SSH_CHANNEL_STATE_OPEN || channel->session->alive == 0);
 ...
 */
void ssh_channel_set_blocking(ssh_channel_t *channel, int blocking)
{
    if(channel == NULL) {
        return;
    }
...
SSH_PACKET_CALLBACK(ssh_packet_channel_success)
{
	...
	// WANGFENG: proxy
	session->command = SSH2_MSG_CHANNEL_SUCCESS;
	channel=channel_from_msg(session, packet);
	if (channel == NULL) {
		SSH_INFO(SSH_LOG_FUNCTIONS, "%s", ssh_get_error(session));
		return SSH_PACKET_USED;
	...
	SSH_INFO(SSH_LOG_PACKET, "Received SSH_CHANNEL_SUCCESS on channel (%d:%d)",
		channel->local_channel, channel->remote_channel);
	if(channel->request_state != SSH_CHANNEL_REQ_STATE_PENDING){
		SSH_INFO(SSH_LOG_RARE, "SSH_CHANNEL_SUCCESS received in incorrect state %d",
...
SSH_PACKET_CALLBACK(ssh_packet_channel_failure)
{
    ...
    SSH_INFO(SSH_LOG_PACKET,
            ...
            channel->remote_channel);
    if(channel->request_state != SSH_CHANNEL_REQ_STATE_PENDING){
        SSH_INFO(SSH_LOG_RARE, "SSH_CHANNEL_FAILURE received in incorrect state %d",
                channel->request_state);
    } else {
...
static int channel_request(ssh_channel_t *channel, const char *request,
    ssh_buffer_t * buffer, int reply)
{
    ssh_session_t *session = channel->session;
    ssh_string_t *req = NULL;
    int rc = SSH_ERROR;
    ...
    if (buffer != NULL)
    {
        if (buffer_add_data(session->out_buffer, buffer_get_rest(buffer), buffer_get_rest_len(buffer)) < 0)
        {
            ssh_set_error_oom(session);
            goto error;
        }
    ...
    //ssh_log(session, "\"SSH2_MSG_CHANNEL_REQUEST(98)\"");
    if (reply == 0) {
...
if(!session->proxy) {
    ...
    case SSH_CHANNEL_REQ_STATE_ACCEPTED:
        SSH_INFO(SSH_LOG_PROTOCOL, "Channel request %s success",request);
        rc = SSH_OK;
        break;
 ...
 *                      to be done again.
 */
int ssh_channel_request_pty_size(ssh_channel_t *channel, 
        const char *terminal, int col, int row)
{
    ssh_session_t *session = NULL;
    ssh_string_t *term = NULL;
 ...
 */
int ssh_channel_request_subsystem(ssh_channel_t *channel, const char *subsys)
{
	ssh_buffer_t *buffer = NULL;
	ssh_string_t *subsystem = NULL;
	int rc = SSH_ERROR;
...
int ssh_channel_request_sftp(ssh_channel_t *channel)
{
	int iRet = SSH_ERROR;
    if(channel != NULL) {
        iRet = ssh_channel_request_subsystem(channel, "sftp");
    }
    return iRet;
}

static ssh_string_t *generate_cookie(void)
{
 ...
 *                      to be done again.
 */
int ssh_channel_request_x11(ssh_channel_t * channel, int single_connection, const char *protocol,
...
static ssh_channel_t *ssh_channel_accept(ssh_session_t *session, int channeltype,
        int timeout_ms, int *destination_port)
{
...
#endif
    ssh_message_t  *msg = NULL;
    ssh_channel_t *channel = NULL;
    ssh_iterator_t *iterator = NULL;
    int t;
    ...
    for (t = timeout_ms; t >= 0; t -= 100)
    {
        ...
        if (session->ssh_message_list)
        {
            ...
            while (iterator) {
                msg = (ssh_message_t *)iterator->data;
 ...
 *                      the server.
 */
ssh_channel_t * ssh_channel_accept_x11(ssh_channel_t *channel, int timeout_ms)
{
...
SSH_PACKET_CALLBACK(ssh_request_success)
{
    ...
    SSH_INFO(SSH_LOG_PACKET, "Received SSH_REQUEST_SUCCESS");
    if(session->global_req_state != SSH_CHANNEL_REQ_STATE_PENDING)
    {
        SSH_INFO(SSH_LOG_RARE, "SSH_REQUEST_SUCCESS received in incorrect state %d",
...
SSH_PACKET_CALLBACK(ssh_request_denied)
{
    ...
    SSH_INFO(SSH_LOG_PACKET, "Received SSH_REQUEST_FAILURE");
    if(session->global_req_state != SSH_CHANNEL_REQ_STATE_PENDING){
    SSH_INFO(SSH_LOG_RARE, "SSH_REQUEST_DENIED received in incorrect state %d",
        session->global_req_state);
    } else {
 ...
 *                      to be done again.
 */
static int global_request(ssh_session_t *session, const char *request,
    ssh_buffer_t *buffer, int reply)
{
    ssh_string_t *req = NULL;
    int rc = SSH_ERROR;
    ...
    SSH_INFO(SSH_LOG_PACKET, "Sent a SSH_MSG_GLOBAL_REQUEST %s", request);
...
if(!session->proxy) {
    ...
    switch(session->global_req_state){
    case SSH_CHANNEL_REQ_STATE_ACCEPTED:
        SSH_INFO(SSH_LOG_PROTOCOL, "Global request %s success", request);
        rc = SSH_OK;
        break;
    case SSH_CHANNEL_REQ_STATE_DENIED:
        SSH_INFO(SSH_LOG_PACKET, "Global request %s failed", request);
        ssh_set_error(session, SSH_REQUEST_DENIED,
 ...
 *                      to be done again.
 **/
int ssh_forward_listen(ssh_session_t *session, const char *address, int port, int *bound_port)
{
    ssh_buffer_t *buffer = NULL;
    ssh_string_t *addr = NULL;
    int rc = SSH_ERROR;
    uint32_t tmp;
 ...
 *         the server
 */
ssh_channel_t *ssh_forward_accept(ssh_session_t *session, int timeout_ms)
{
 ...
 *         the server
 */
ssh_channel_t *ssh_channel_accept_forward(ssh_session_t *session, int timeout_ms, int *destination_port)
{
 ...
 *                      to be done again.
 */
int ssh_forward_cancel(ssh_session_t *session, const char *address, int port)
{
    ssh_buffer_t * buffer = NULL;
    ssh_string_t * addr = NULL;
    int rc = SSH_ERROR;
 ...
 */
int ssh_channel_request_env(ssh_channel_t *channel, const char *name, const char *value)
{
    ssh_buffer_t * buffer = NULL;
    ssh_string_t * str = NULL;
    int rc = SSH_ERROR;
 ...
 * @see ssh_channel_request_shell()
 */
int ssh_channel_request_exec(ssh_channel_t *channel, const char *cmd)
{
    ssh_buffer_t * buffer = NULL;
    ssh_string_t * command = NULL;
    int rc = SSH_ERROR;
 ...
 */
int ssh_channel_request_send_signal(ssh_channel_t *channel, const char *sig)
{
    ssh_buffer_t *buffer = NULL;
    ssh_string_t *encoded_signal = NULL;
    int rc = SSH_ERROR;
 ...
 * @see ssh_channel_read
 */
int channel_read_buffer(ssh_channel_t *channel, ssh_buffer_t *buffer, 
 ...
 *          channel_read_buffer().
 */
int ssh_channel_read(ssh_channel_t *channel, void *dest, uint32_t count, int is_stderr)
{
 ...
 *          channel_read_buffer().
 */
int ssh_channel_read_timeout(ssh_channel_t *channel,
        void *dest, uint32_t count, int is_stderr, int timeout)
{
    ssh_session_t *session = NULL;
    ssh_buffer_t *stdbuf = NULL;
    uint32_t len;
    ...
    rc = ssh_handle_packets_termination(session,
            timeout, ssh_channel_read_termination, &ctx);
    if (rc == SSH_ERROR){
        return rc;
    }
    if (channel->session->session_state == SSH_SESSION_STATE_ERROR){
 ...
 * @see ssh_channel_is_eof()
 */
int ssh_channel_read_nonblocking(ssh_channel_t * channel, void *dest, uint32_t count,
    int is_stderr)
{
    ssh_session_t * session;
    int to_read;
 ...
 * @see ssh_channel_is_eof()
 */
int ssh_channel_poll(ssh_channel_t *channel, int is_stderr)
{
    ssh_buffer_t *stdbuf = NULL;
 ...
 * @see ssh_channel_is_eof()
 */
int ssh_channel_poll_timeout(ssh_channel_t *channel, int timeout, int is_stderr)
{
    ssh_session_t *session = NULL;
    ssh_buffer_t *stdbuf = NULL;
    struct ssh_channel_read_termination_struct ctx;
 ...
 */
ssh_session_t * ssh_channel_get_session(ssh_channel_t *channel)
{
    if(channel == NULL) {
        return NULL;
    }
 ...
 */
static int channel_protocol_select(ssh_channel_t * *rchans, ssh_channel_t * *wchans,
        ssh_channel_t * *echans, ssh_channel_t * *rout, ssh_channel_t * *wout, ssh_channel_t * *eout)
{
    ssh_channel_t *chan = NULL;
    int i;
    int j = 0;
...
static int count_ptrs(ssh_channel_t **ptrs)
{
    int c;
 ...
 *                     function.
 */
int ssh_channel_select(ssh_channel_t * *readchans, ssh_channel_t * *writechans,
    ssh_channel_t * *exceptchans, struct timeval *timeout)
{
    ssh_channel_t * *rchans, **wchans, **echans;
    ssh_channel_t *dummy = NULL;
    ssh_event_t *event = NULL;
    int rc = SSH_OK;
    int i;
    int tm, tm_base;
    int firstround=1;
    ...
    rchans = malloc(sizeof(ssh_channel_t *) * (count_ptrs(readchans) + 1));
    if (rchans == NULL) {
        return SSH_ERROR;
    }
    
    wchans = malloc(sizeof(ssh_channel_t *) * (count_ptrs(writechans) + 1));
    if (wchans == NULL) {
        SAFE_FREE(rchans);
        return SSH_ERROR;
    }
    
    echans = malloc(sizeof(ssh_channel_t *) * (count_ptrs(exceptchans) + 1));
    if (echans == NULL) {
        SAFE_FREE(rchans);
        SAFE_FREE(wchans);
        return SSH_ERROR;
    ...
    memcpy(readchans, rchans, (count_ptrs(rchans) + 1) * sizeof(ssh_channel_t * ));
    memcpy(writechans, wchans, (count_ptrs(wchans) + 1) * sizeof(ssh_channel_t * ));
    memcpy(exceptchans, echans, (count_ptrs(echans) + 1) * sizeof(ssh_channel_t * ));
    SAFE_FREE(rchans);
    SAFE_FREE(wchans);
    SAFE_FREE(echans);
    if(event) {
 ...
 * @see ssh_channel_read()
 */
int ssh_channel_write_stderr(ssh_channel_t * channel, const void *data, uint32_t len)
{
 ...
 *          use channel_read and channel_write for this.
 */
int ssh_channel_open_reverse_forward(ssh_channel_t * channel, const char *remotehost,
    int remoteport, const char *sourcehost, int localport)
{
    ssh_session_t *session = NULL;
    ssh_buffer_t *payload = NULL;
 ...
 *          use channel_read and channel_write for this.
 */
int ssh_channel_open_x11(ssh_channel_t *channel, const char *orig_addr, int orig_port)
{
    ssh_session_t *session = NULL;
    ssh_buffer_t *payload = NULL;
    ssh_string_t *str = NULL;
 ...
 */
int ssh_channel_request_send_exit_status(ssh_channel_t *channel, int exit_status) {
    ssh_buffer_t *buffer = NULL;
    int rc = SSH_ERROR;
 ...
 *                      (including attempts to send signal via SSH-v1 session).
 */
int ssh_channel_request_send_exit_signal(ssh_channel_t *channel, const char *sig,
