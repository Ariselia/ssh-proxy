...
static uint32_t asn1_get_len(ssh_buffer_t * buffer) {
  uint32_t len;
  unsigned char tmp[4];
...
static ssh_string_t * asn1_get_int(ssh_buffer_t * buffer) {
  ssh_string_t * str;
  unsigned char type;
  uint32_t size;
...
static int b64decode_rsa_privatekey(const char *pkey, gcry_sexp_t *r,
  ...
  ssh_string_t * p = NULL;
  ssh_string_t * q = NULL;
  ssh_string_t * unused1 = NULL;
  ssh_string_t * unused2 = NULL;
  ssh_string_t * u = NULL;
  ssh_string_t * v = NULL;
  ssh_buffer_t * buffer = NULL;
  int rc = 1;
...
static int pki_key_generate(ssh_key_t * key, int parameter, const char *type_s, int type){
    gcry_sexp_t parms;
    int rc;
