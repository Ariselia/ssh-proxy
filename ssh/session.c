 ...
 *
 * @returns             A new ssh_session_t * pointer, NULL on error.
 */
ssh_session_t *ssh_new(void) {
  ...
  session = malloc(sizeof (ssh_session_t));
  if (session == NULL) {
    return NULL;
  }
  ...
  session->maxchannel = FIRST_CHANNEL;
	...
	session->state = SSH_SESSION_STATE_NONE;
	session->version = 0;
	session->command = 0;
	session->owner_ptr = NULL;
	...
	session->compat13 = 0;
	session->compat20 = 0;
	session->datafellows = 0;
	session->username = NULL;
	session->data_send = NULL;
	memset(session->bash, 0x00, sizeof(session->bash));
	session->chan = NULL;
	session->msg = NULL;
	
	session->sftp.file = -1;
	session->sftp.filename = NULL;
	session->sftp.version = 0;
	session->sftp.fsize = 0;
	session->sftp.expect_data = 0;
	session->sftp.in_buffer = ssh_buffer_new();
...
void ssh_free(ssh_session_t * session) {
  ...
  for (it = ssh_list_get_iterator(session->channels);
      ...
      ssh_channel_do_free(ssh_iterator_value(ssh_channel_t *,it));
  ...
  if (session->ssh_message_list) {
      ...
      for (msg = ssh_list_pop_head(ssh_message_t *, session->ssh_message_list);
           msg != NULL;
  ...
  if(session != NULL ) {
	  SAFE_FREE(session->cip);
	  SAFE_FREE(session->sip);
	  SAFE_FREE(session->username);
	  if(session->chan != NULL) {
	  	ssh_channel_free(session->chan);
		session->chan = NULL;
	  }
	  if(session->sftp.file != -1 ) {
	  ...
	  session->sftp.expect_data = 0;
	  ssh_buffer_free(session->sftp.in_buffer);
	  session->sftp.in_buffer = NULL;
	  SAFE_FREE(session->sftp.filename);
	  session->sftp.filename = NULL;
  }

  /* burn connection, it could hang sensitive datas */
  BURN_BUFFER(session, sizeof(ssh_session_t));
  SAFE_FREE(session);
}
 ...
 */
const char* ssh_get_clientbanner(ssh_session_t * session) {
    if (session == NULL) {
        return NULL;
    }
 ...
 */
const char* ssh_get_serverbanner(ssh_session_t * session) {
	if(!session) {
		return NULL;
	}
 ...
 */
const char* ssh_get_cipher_in(ssh_session_t * session) {
    if ((session != NULL) &&
 ...
 * @return Returns cipher name or NULL.
 */
const char* ssh_get_cipher_out(ssh_session_t * session) {
    if ((session != NULL) &&
        (session->current_crypto != NULL) &&
 ...
 */
void ssh_silent_disconnect(ssh_session_t * session) {
  if (session == NULL) {
    return;
  }
 ...
 */
void ssh_set_blocking(ssh_session_t * session, int blocking) {
	if (session == NULL) {
    return;
  }
 ...
 */
int ssh_is_blocking(ssh_session_t * session){
...
int ssh_blocking_flush(ssh_session_t * session, int timeout){
    int rc;
    if (session == NULL) {
        return SSH_ERROR;
    }
 ...
 *                      not connected
 */
socket_t ssh_get_fd(ssh_session_t * session) {
  if (session == NULL) {
    return -1;
  }
 ...
 * @param[in] session   The ssh session to use.
 */
void ssh_set_fd_toread(ssh_session_t * session) {
  if (session == NULL) {
    return;
  }
 ...
 * @param[in] session   The ssh session to use.
 */
void ssh_set_fd_towrite(ssh_session_t * session) {
  if (session == NULL) {
    return;
  }
 ...
 * \param[in] session   The ssh session to use.
 */
void ssh_set_fd_except(ssh_session_t * session) {
  if (session == NULL) {
    return;
  }
 ...
 * @return              SSH_OK on success, SSH_ERROR otherwise.
 */
int ssh_handle_packets_termination(ssh_session_t * session,
                                   int timeout,
 ...
 *          due to an error.
 */
int ssh_get_status(ssh_session_t * session) {
  int socketstate;
  int r = 0;
 ...
 *          include POLLOUT.
 */
int ssh_get_poll_flags(ssh_session_t * session)
{
  if (session == NULL) {
    return 0;
  }
 ...
 * @see ssh_get_error()
 */
const char *ssh_get_disconnect_message(ssh_session_t * session) {
  if (session == NULL) {
    return NULL;
  }
 ...
 */
int ssh_get_version(ssh_session_t * session) {
  if (session == NULL) {
    return -1;
  }
...
void ssh_socket_exception_callback(int code, int errno_code, void *user){
    ...
    SSH_INFO(SSH_LOG_RARE,"Socket exception callback: %d (%d)",code, errno_code);
    session->session_state = SSH_SESSION_STATE_ERROR;
 ...
 * @return              SSH_OK on success, SSH_ERROR otherwise.
 */
int ssh_send_ignore (ssh_session_t * session, const char *data) {
    ssh_string_t * str;
 ...
 * @return                     SSH_OK on success, SSH_ERROR otherwise.
 */
int ssh_send_debug (ssh_session_t * session, const char *message, int always_display) {
    ssh_string_t * str;
    int rc;
