...
int channel_open_session1(ssh_channel_t * chan) {
  ...
  SSH_INFO(SSH_LOG_PACKET, "Opened a SSH1 channel session");
...
int channel_request_pty_size1(ssh_channel_t * channel, const char *terminal, int col,
    int row) {
  ssh_session_t * session;
  ...
  SSH_INFO(SSH_LOG_FUNCTIONS, "Opening a ssh1 pty");
  ...
  switch(channel->request_state){
    ...
    case SSH_CHANNEL_REQ_STATE_ACCEPTED:
      channel->request_state=SSH_CHANNEL_REQ_STATE_NONE;
      SSH_INFO(SSH_LOG_RARE, "PTY: Success");
      return SSH_OK;
    case SSH_CHANNEL_REQ_STATE_DENIED:
      channel->request_state=SSH_CHANNEL_REQ_STATE_NONE;
      ssh_set_error(session, SSH_REQUEST_DENIED,
          "Server denied PTY allocation");
      SSH_INFO(SSH_LOG_RARE, "PTY: denied\n");
      return SSH_ERROR;
  }
  // Not reached
  return SSH_ERROR;
...
int channel_change_pty_size1(ssh_channel_t * channel, int cols, int rows) {
  ...
  SSH_INFO(SSH_LOG_PROTOCOL, "Change pty size send");
  while(channel->request_state==SSH_CHANNEL_REQ_STATE_PENDING){
    ssh_handle_packets(session, SSH_TIMEOUT_INFINITE);
  }
  switch(channel->request_state){
    ...
    case SSH_CHANNEL_REQ_STATE_ACCEPTED:
      channel->request_state=SSH_CHANNEL_REQ_STATE_NONE;
      SSH_INFO(SSH_LOG_PROTOCOL, "pty size changed");
      return SSH_OK;
    case SSH_CHANNEL_REQ_STATE_DENIED:
      channel->request_state=SSH_CHANNEL_REQ_STATE_NONE;
      SSH_INFO(SSH_LOG_RARE, "pty size change denied");
...
int channel_request_shell1(ssh_channel_t * channel) {
  ...
  SSH_INFO(SSH_LOG_RARE, "Launched a shell");
...
int channel_request_exec1(ssh_channel_t * channel, const char *cmd) {
  ...
  SSH_INFO(SSH_LOG_RARE, "Executing %s ...", cmd);

  return 0;
}

SSH_PACKET_CALLBACK(ssh_packet_data1){
    ssh_channel_t * channel = ssh_get_channel1(session);
    ssh_string_t * str = NULL;
    ...
    if (str == NULL) {
      SSH_INFO(SSH_LOG_FUNCTIONS, "Invalid data packet !\n");
...
ssh_channel_t * ssh_get_channel1(ssh_session_t * session){
  ...
  if(session->channels != NULL){
    ...
    if(it)
      return ssh_iterator_value(ssh_channel_t *, it);
  }
  return NULL;
}
#endif /* WITH_SSH1 */
