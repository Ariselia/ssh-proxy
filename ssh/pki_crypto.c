...
static ssh_string_t * make_ecpoint_string(const EC_GROUP *g,
                                      const EC_POINT *p)
{
    ssh_string_t * s;
    size_t len;
 ...
 */
static ssh_string_t * _RSA_do_sign(const unsigned char *digest,
                               int dlen,
                               RSA *privkey)
{
...
static ssh_string_t * pki_dsa_signature_to_blob(const ssh_signature sig)
{
    char buffer[40] = { 0 };
...
ssh_string_t * pki_signature_to_blob(const ssh_signature sig)
{
    ssh_string_t * r;
    ssh_string_t * s;
    ssh_string_t * sig_blob = NULL;
...
static ssh_signature pki_signature_from_rsa_blob(const ssh_key_t * pubkey,
