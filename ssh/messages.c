...
static ssh_message_t * ssh_message_new(ssh_session_t * session){
  ssh_message_t * msg = malloc(sizeof(ssh_message_t));
  if (msg == NULL) {
    return NULL;
  }
  ZERO_STRUCTP(msg);
 ...
 */
static int ssh_message_reply_default(ssh_message_t * msg) {
  SSH_INFO(SSH_LOG_FUNCTIONS, "Reporting unknown packet");
...
static int ssh_execute_server_request(ssh_session_t * session, ssh_message_t * msg)
{
...
static int ssh_execute_client_request(ssh_session_t * session, ssh_message_t * msg)
{
    ssh_channel_t * channel = NULL;
    int rc = SSH_AGAIN;
...
/** @internal
 * Executes the callbacks defined in session->server_callbacks, out of an ssh_message_t *
 * I don't like ssh_message_t * interface but it works.
 * @returns SSH_OK if the message has been handled, or SSH_AGAIN otherwise.
 */
static int ssh_execute_server_callbacks(ssh_session_t * session, ssh_message_t * msg){
    int rc = SSH_AGAIN;
	// WANGFENG: callback
...
static int ssh_execute_message_callback(ssh_session_t * session, ssh_message_t * msg) {
	int ret;
 ...
 */
void ssh_message_queue(ssh_session_t * session, ssh_message_t * message)
{
    if (message != NULL) {
	#ifdef WITH_SERVER
		int ret;
 ...
 */
ssh_message_t * ssh_message_pop_head(ssh_session_t * session){
  	ssh_message_t *msg = NULL;
  	ssh_iterator_t *i = NULL;
  	if(session->ssh_message_list == NULL) {
    	return NULL;
  	}
  	i = ssh_list_get_iterator(session->ssh_message_list);
  	if(i != NULL){
    	msg = ssh_iterator_value(ssh_message_t *, i);
 ...
 */
ssh_message_t * ssh_message_get(ssh_session_t * session) {
  	ssh_message_t * msg = NULL;
  	int rc;
  	...
  	msg = ssh_list_pop_head(ssh_message_t *, session->ssh_message_list);
 ...
 */
int ssh_message_type(ssh_message_t * msg) {
  if (msg == NULL) {
    return -1;
  }
 ...
 * @return              The message type or -1 on error.
 */
int ssh_message_subtype(ssh_message_t *msg) {
  	if (msg == NULL) {
    	return -1;
  	}
 ...
 */
void ssh_message_free(ssh_message_t * msg){
  	if (msg == NULL) {
    	return;
  	}
...
SSH_PACKET_CALLBACK(ssh_packet_service_request)
{
  	...
  	SSH_INFO(SSH_LOG_PACKET, "Received a SERVICE_REQUEST for service %s", service_c);
 ...
 */
static ssh_buffer_t *
ssh_msg_userauth_build_digest(ssh_session_t *session,
		ssh_message_t *msg, const char *service)
{
    ssh_crypto_t *crypto =
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_request)
{
  ...
  // WANGFENG: proxy, try forward packet
  //buffer_add_u8(msg->packet, SSH2_MSG_USERAUTH_REQUEST);
  //buffer_add_buffer(msg->packet, packet);
  ...
  if (strncmp(method, "keyboard-interactive", method_size) == 0) {
    ssh_string_t * lang = NULL;
    ssh_string_t * submethods = NULL;
  ...
  if (strncmp(method, "publickey", method_size) == 0) {
    ...
    if(has_sign) {
        ...
        ssh_buffer_t * digest = NULL;

        sig_blob = buffer_get_ssh_string(packet);
        if(sig_blob == NULL) {
            SSH_INFO(SSH_LOG_PACKET, "Invalid signature packet from peer");
        ...
        if (digest == NULL) {
            ...
            SSH_INFO(SSH_LOG_PACKET, "Failed to get digest");
        ...
        if (rc < 0) {
            SSH_INFO(SSH_LOG_PACKET, "Received an invalid  signature from peer");
            msg->auth_request.signature_state = SSH_PUBLICKEY_STATE_WRONG;
            goto error;
        }

        SSH_INFO(SSH_LOG_PACKET, "Valid signature received");
  ...
  if (strncmp(method, "gssapi-with-mic", method_size) == 0) {
     ...
     SSH_INFO(SSH_LOG_PACKET, "gssapi: %d OIDs", n_oid);
     oids = calloc(n_oid, sizeof(ssh_string_t *));
     if (oids == NULL){
    	 ssh_set_error_oom(session);
    	 goto error;
     }
     for (i=0;i<(int) n_oid;++i){
    	 ...
    	 if(session->common.log_verbosity >= SSH_LOG_PACKET){
    		 ...
    		 SSH_INFO(SSH_LOG_PACKET,"gssapi: OID %d: %s",i, hexa);
    		 SAFE_FREE(hexa);
    	 }
     }
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_info_response){
  ...
  if (session->kbdint == NULL) {
    SSH_INFO(SSH_LOG_PROTOCOL, "Warning: Got a keyboard-interactive "
  ...
  SSH_INFO(SSH_LOG_PACKET,"kbdint: %d answers",nanswers);
  ...
  if(nanswers != session->kbdint->nprompts) {
    ...
    SSH_INFO(SSH_LOG_PROTOCOL, "Warning: Number of prompts and answers"
  ...
  for (i = 0; i < nanswers; i++) {
	...
	trace_out("session->kbdint->answers[i] = %s", session->kbdint->answers[i]);
    ssh_string_free(tmp);
...
SSH_PACKET_CALLBACK(ssh_packet_channel_open){
  ssh_message_t * msg = NULL;
  ssh_string_t * type_s = NULL, *originator = NULL, *destination = NULL;
  char *type_c = NULL;
  ...
  trace_out("s = %d, w = %d, p = %d", msg->channel_request_open.sender, msg->channel_request_open.window, msg->channel_request_open.packet_size);
...
int ssh_message_channel_request_open_reply_accept_channel(ssh_message_t * msg, ssh_channel_t * chan) {
	...
	//ssh_log(session, "\"SSH2_MSG_CHANNEL_OPEN_CONFIRMATION(91)\"");
    return rc;
}


ssh_channel_t * ssh_message_channel_request_open_reply_accept(ssh_message_t * msg) {
	ssh_channel_t * chan;
	int rc;
 ...
 */
int ssh_message_handle_channel_request(ssh_session_t * session, ssh_channel_t * channel, ssh_buffer_t * packet,
    const char *request, uint8_t want_reply)
{
  	...
  	if (strcmp(request, "subsystem") == 0) {
		...
		trace_out("subsystem----------------------------------------------- %s", subsys_c);
    	if (subsys_c == NULL) {
      		ssh_set_error_oom(session);
  	...
  	if (strcmp(request, "x11-req") == 0) {
    	ssh_string_t * auth_protocol = NULL;
    	ssh_string_t * auth_cookie = NULL;
...
int ssh_message_channel_request_reply_success(ssh_message_t * msg) {
  	uint32_t channel;
  	int rc;
  	if (msg == NULL) {
    	return SSH_ERROR;
  	...
  	if (msg->channel_request.want_reply) {
		...
		//ssh_log(msg->session, "\"SSH2_MSG_CHANNEL_SUCCESS(99)\"");
		return rc;
  	}
...
SSH_PACKET_CALLBACK(ssh_packet_global_request)
{
    ...
    SSH_INFO(SSH_LOG_PROTOCOL,"Received SSH_MSG_GLOBAL_REQUEST packet");
    ...
    if (request && strcmp(request, "tcpip-forward") == 0) {
        ...
        SSH_INFO(SSH_LOG_PROTOCOL, "Received SSH_MSG_GLOBAL_REQUEST %s %d %s:%d", request, want_reply, bind_addr, bind_port);

        if(ssh_callbacks_exists(session->common.callbacks, global_request_function)) {
            SSH_INFO(SSH_LOG_PROTOCOL, "Calling callback for SSH_MSG_GLOBAL_REQUEST %s %d %s:%d", request, want_reply, bind_addr, bind_port);
        ...
        SSH_INFO(SSH_LOG_PROTOCOL, "UNKNOWN SSH_MSG_GLOBAL_REQUEST %s %d", request, want_reply);
