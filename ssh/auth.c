 ...
 * again is necessary
 */
static int ssh_userauth_request_service(ssh_session_t * session) {
    int rc;
 ...
 */
static int ssh_userauth_get_response(ssh_session_t * session) {
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_failure){
  	...
  	session->command = SSH2_MSG_USERAUTH_FAILURE;
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_success){
  	...
  	SSH_INFO(SSH_LOG_DEBUG, "Authentication successful");
  	SSH_INFO(SSH_LOG_TRACE, "Received SSH_USERAUTH_SUCCESS");
  	...
  	if(session->current_crypto && session->current_crypto->delayed_compress_out){
      	SSH_INFO(SSH_LOG_DEBUG, "Enabling delayed compression OUT");
  		session->current_crypto->do_compress_out=1;
  	}
  	if(session->current_crypto && session->current_crypto->delayed_compress_in){
      	SSH_INFO(SSH_LOG_DEBUG, "Enabling delayed compression IN");
  		session->current_crypto->do_compress_in=1;
  	}
  	session->command = SSH2_MSG_USERAUTH_SUCCESS;
  	return SSH_PACKET_USED;
}
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_pk_ok)
{
	int rc;
  	...
  	SSH_INFO(SSH_LOG_TRACE, "Received SSH_USERAUTH_PK_OK/INFO_REQUEST/GSSAPI_RESPONSE");
	
  	if(session->auth_state == SSH_AUTH_STATE_KBDINT_SENT){
    	/* Assuming we are in keyboard-interactive context */
    	SSH_INFO(SSH_LOG_TRACE,
    	...
    	SSH_INFO(SSH_LOG_TRACE, "Assuming SSH_USERAUTH_PK_OK");
    	rc = SSH_PACKET_USED;
  	}
	// WANGFENG: proxy
	session->command = SSH2_MSG_USERAUTH_PK_OK;
  	return rc;
}
 ...
 * before you connect to the server.
 */
int ssh_userauth_try_publickey(ssh_session_t * session,
                               const char *username,
 ...
 * before you connect to the server.
 */
int ssh_userauth_publickey(ssh_session_t * session,
                           const char *username,
...
#ifndef _WIN32
static int ssh_userauth_agent_publickey(ssh_session_t * session,
                                        const char *username,
                                        ssh_key_t * pubkey)
{
    ssh_string_t * str;
 ...
 * before you connect to the server.
 */
int ssh_userauth_agent(ssh_session_t * session,
                       const char *username) {
    int rc = SSH_AUTH_ERROR;
 ...
 * before you connect to the server.
 */
int ssh_userauth_publickey_auto(ssh_session_t * session,
                                const char *username,
 ...
 */
int ssh_userauth_password(ssh_session_t * session,
                          const char *username,
                          const char *password) {
    ssh_string_t * str;
    int rc;
...
#ifndef _WIN32
/* LEGACY */
int ssh_userauth_agent_pubkey(ssh_session_t * session,
                              const char *username,
 ...
 */
static int ssh_userauth_kbdint_init(ssh_session_t * session,
                                    const char *username,
 ...
 */
static int ssh_userauth_kbdint_send(ssh_session_t *session)
{
    ssh_string_t *answer = NULL;
    uint32_t i;
    int rc;
...
{
  ssh_string_t *name; /* name of the "asking" window showed to client */
  ssh_string_t *instruction;
  ssh_string_t *tmp;
  uint32_t nprompts;
  uint32_t i;
  (void)user;
 ...
 */
int ssh_userauth_kbdint(ssh_session_t * session, const char *user,
    const char *submethods) {
    int rc = SSH_AUTH_ERROR;
 ...
 */
int ssh_userauth_kbdint_getnprompts(ssh_session_t *session) {
  	if(session == NULL) {
    	return SSH_ERROR;
  	}
  	if(session->kbdint == NULL) {
 ...
 */
const char *ssh_userauth_kbdint_getname(ssh_session_t * session) {
  	if(session == NULL) {
    	return NULL;
  	}
  	if(session->kbdint == NULL) {
...
const char *ssh_userauth_kbdint_getinstruction(ssh_session_t * session) {
  	if(session == NULL) {
    	return NULL;
  	}
  	if(session->kbdint == NULL) {
 ...
 * @endcode
 */
const char *ssh_userauth_kbdint_getprompt(ssh_session_t * session, unsigned int i,
    char *echo) {
  	if(session == NULL) {
    	return NULL;
  	}
 ...
 */
int ssh_userauth_kbdint_getnanswers(ssh_session_t * session) {
 ...
 */
const char *ssh_userauth_kbdint_getanswer(ssh_session_t *session, unsigned int i) {
  	if(session==NULL || session->kbdint == NULL
 ...
 * @return              0 on success, < 0 on error.
 */
int ssh_userauth_kbdint_setanswer(ssh_session_t * session, unsigned int i,
    const char *answer) {
  	if (session == NULL) {
    	return -1;
  	}
 ...
 *                            later.
 */
int ssh_userauth_gssapi(ssh_session_t * session) {
	...
	SSH_INFO(SSH_LOG_PROTOCOL, "Authenticating with gssapi-with-mic");
	session->auth_state = SSH_AUTH_STATE_NONE;
