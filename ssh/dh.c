...
/*
static void sha_add(ssh_string_t * str,SHACTX ctx){
...
int make_sessionid(ssh_session_t * session) {
  ssh_string_t * num = NULL;
  ssh_string_t * str = NULL;
  ssh_buffer_t * server_hash = NULL;
  ssh_buffer_t * client_hash = NULL;
  ...
  if(session->next_crypto->kex_type == SSH_KEX_DH_GROUP1_SHA1 ||
    ...
    if(session->next_crypto->ecdh_client_pubkey == NULL ||
        ...
        SSH_INFO(SSH_LOG_WARNING, "ECDH parameted missing");
        goto error;
    }
...
static int generate_one_key(ssh_string_t * k,
    ssh_crypto_t *crypto, unsigned char *output, char letter) {
  ssh_mac_ctx ctx;
...
int generate_session_keys(ssh_session_t * session) {
  ssh_string_t * k_string = NULL;
  ssh_mac_ctx ctx = NULL;
  ssh_crypto_t *crypto = session->next_crypto;
  unsigned char *tmp;
  int rc = -1;
 ...
 */
int ssh_get_pubkey_hash(ssh_session_t * session, unsigned char **hash) {
  ssh_string_t * pubkey;
  MD5CTX ctx;
  unsigned char *h;
 ...
 * @see ssh_key_free()
 */
int ssh_get_publickey(ssh_session_t * session, ssh_key_t * *key)
{
    if (session==NULL ||
 ...
 */
int ssh_get_publickey_hash(const ssh_key_t * key,
